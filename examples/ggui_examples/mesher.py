...
class Mesher:
    def __init__(self, space_min, space_max, sdf_cell_size, mesh_cell_size):
        ...
        self.sdf_grid_size = ceil(space_size[0] / sdf_cell_size) + 3, ceil(
            space_size[1] / sdf_cell_size) + 3, ceil(
                space_size[2] / sdf_cell_size) + 3
        self.mesh_grid_size = ceil(space_size[0] / mesh_cell_size) + 2, ceil(
            space_size[1] / mesh_cell_size) + 2, ceil(
                space_size[2] / mesh_cell_size) + 2
    ...
    def mesh_kernel(self):
        ...
        for cell in ti.grouped(ti.ndrange(*self.mesh_grid_size)):
            ...
            for i in range(8):
                this_cell = cell + ti.Vector([
                    self.mc_vert_coords[i, 0], self.mc_vert_coords[i, 1],
                    self.mc_vert_coords[i, 2]
                ])
            ...
            while vertex_id < 15:
                ...
                coords_a = float(
                    ti.Vector([
                        self.mc_vert_coords[vert_a,
                                            0], self.mc_vert_coords[vert_a, 1],
                        self.mc_vert_coords[vert_a, 2]
                    ]))
                coords_b = float(
                    ti.Vector([
                        self.mc_vert_coords[vert_b,
                                            0], self.mc_vert_coords[vert_b, 1],
                        self.mc_vert_coords[vert_b, 2]
                    ]))
