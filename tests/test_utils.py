...
Note that the function signature for "python_function_name" has to be "def xxxx(folder_path):", which will be imported and executed by run_tests.py. After that, run_tests.py will execute the corresponding C++ test named "cpp_test_name".

The temporary directory where serialized cache file stays will be generated by run_tests.py, and directly passed as an argument to "python_function_name" upon execution. As for C++ test, you may fetch this directory from environment variable "TAICHI_AOT_FOLDER_PATH".
"""
    print(message)
