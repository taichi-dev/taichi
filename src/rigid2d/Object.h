...
class Object {
 public:
  ...
  Matrix3x3 GetTransformToWorld() { return transformToWorld; }
  Vector2D GetTransformToWorld(Vector2D vec) { return transformToWorld(vec); }

  Matrix3x3 GetTransformToWorldInverse() { return transformToWorldInverse; }
  ...
  Shape *GetShape(int id) { return shapes[id]; }
  Object() { Init(); }
  Object(Shape *shape) {
    Init();
    AddShape(shape);
  ...
  void Update() {
    ...
    for (int i = 0; i < n; i++)
      base_inertia +=
          shapes[i]->inertia +
          shapes[i]->mass * shapes[i]->centroidPosition.GetLength2();
    if (fixed) {
      mass = DBL_INF;
      inertia = DBL_INF;
    } else
      mass = base_mass, inertia = base_inertia;
    invMass = 1.0 / mass;
  ...
  void ApplyTorque(double torque) { angularVelocity += torque * invInertia; }
  ...
  void Proceed(double T) {
    ...
    if (sgn(linearVelocity.GetLength2()))
      linearVelocity -= (1 - pow(1 - linearResistance, T / timeInterval)) *
                        linearVelocity.GetLength2() *
                        linearVelocity.GetDirection();
    angularVelocity -= (1 - pow(1 - rotationResistance, T / timeInterval)) *
                       sqr(angularVelocity);
    correctiveLinearVelocity = Vector2D(0, 0, 0);
    correctiveAngularVelocity = 0.0;
    ResetTransformToWorld();
  ...
  Vector2D GetPointVelocity(Vector2D p) {
    if (fixed)
      ...
      return linearVelocity + Vector2D(-angularVelocity * (p.y - position.y),
                                       angularVelocity * (p.x - position.x), 0);
  }
  void Redraw() {
    for (int i = 0; i < (int)shapes.size(); i++)
      shapes[i]->Redraw();
