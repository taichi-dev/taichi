...
def compile_graph_aot(arch):
    ...
    tmpdir = pathdir + "/module.tcm"

    mod = ti.aot.Module()
    mod.add_graph('run_graph', run_graph)
    mod.archive(tmpdir)
    os.rename(tmpdir, pathdir + "/module.zip")
    zip_file = zipfile.ZipFile(pathdir + "/module.zip")
    ...
    zip_files(pathdir + '/module')
    shutil.rmtree(pathdir + '/module')
    os.rename(pathdir + "/module.zip", tmpdir)
