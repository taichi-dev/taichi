...
// Check here for a more understandable version:
...
void BidirectionalRenderer::initialize(const Config &config) {
  ...
  this->max_eye_events =
      std::min(this->max_eye_events, this->max_path_length + 1);
  this->max_light_events =
...
void BidirectionalRenderer::trace(Path &path,
                                  Ray r,
                                  int depth,
                                  int max_depth,
                                  StateSequence &rand) {
...
bool BidirectionalRenderer::connectable(int num_eye_vertices,
                                        int num_light_vertices,
                                        const Vertex &eye_end,
                                        const Vertex &light_end) {
  const Vector3 dir = normalize(light_end.pos - eye_end.pos);
  if ((num_eye_vertices == 1) && (num_light_vertices >= 1)) {
    // Light tracing
    if (SurfaceEventClassifier::is_delta(light_end.event) ||
        dot(light_end.normal, dir) > -eps ||
    ...
    if (!SurfaceEventClassifier::is_delta(light_end.event) &&
        !SurfaceEventClassifier::is_delta(eye_end.event)) {
      if (light_end.triangle_id ==
              eye_end.triangle_id ||  // on the same triangle?
...
double BidirectionalRenderer::path_pdf(const Path &path,
  ...
  const int is_vm =
      ((int)path.size() - 1 == num_eye_vertices + num_light_vertices - 2);
  ...
  for (int i = -1; i <= num_light_vertices - 2 - is_vm; i++) {
    if (i == -1) {
      ...
      Vector3 in_dir =
          normalize(path[path_length - 1].pos - path[path_length].pos);
      ...
      if (path[path_length - i].connected) {
        Vector3 in_dir = normalize(path[path_length - (i - 1)].pos -
                                   path[path_length - i].pos);
        Vector3 out_dir = normalize(path[path_length - (i + 1)].pos -
                                    path[path_length - i].pos);
      ...
      p = p *
          direction_to_area(path[path_length - i], path[path_length - (i + 1)]);
    }
  }
  if (p == 0.0)
    return p;  // Shortcut
  if (is_vm) {
    ...
    if (SurfaceEventClassifier::is_delta(eye_end.event))
      p = 0;
    else {
      p *= light_end.pdf * direction_to_area(light_end, eye_end) *
           vm_pdf_constant;
    }
  }
  return p;
...
double BidirectionalRenderer::path_total_pdf(const Path &path,
  ...
  if (including_connection) {
    ...
    for (int num_eye_vertices = 1; num_eye_vertices <= path_length + 1;
         num_eye_vertices++) {
      int num_light_vertices = (path_length + 1) - num_eye_vertices;
      if (num_eye_vertices > max_eye_events ||
          num_light_vertices > max_light_events) {
        continue;
      }
      if (num_eye_vertices >= 2 &&
          SurfaceEventClassifier::is_delta(path[num_eye_vertices - 1].event)) {
        continue;
      }
      if (num_light_vertices >= 2 &&
          SurfaceEventClassifier::is_delta(path[num_eye_vertices].event)) {
        continue;
      }
  ...
  if (merging_factor > 0) {
    ...
    for (int num_eye_vertices = 2; num_eye_vertices <= path_length;
         num_eye_vertices++) {
...
PathContribution BidirectionalRenderer::connect(const Path &eye_path,
                                                const Path &light_path,
                                                const int num_eye_vert_spec,
                                                const int num_light_vert_spec,
                                                const int merging_factor) {
  PathContribution result;
  bool specified = (num_eye_vert_spec != -1) && (num_light_vert_spec != -1);

  for (int path_length = min_path_length; path_length <= max_path_length;
       path_length++) {
    Path full_path;
    ...
    for (int num_eye_vertices = 1; num_eye_vertices <= path_length + 1;
      ...
      if (num_eye_vertices > (int)eye_path.size())
        continue;
      if (num_light_vertices > (int)light_path.size())
        continue;
      ...
      if (num_eye_vertices == 0) {
        ...
        bool valid = scene->get_triangle_emission(eye_end.triangle_id) > 0 &&
                     eye_end.front;
        if (!valid) {
          continue;
        }
        ...
        if (!connectable(num_eye_vertices, num_light_vertices, eye_end,
                         light_end)) {
          continue;
        }
      }
      ...
      for (int i = 0; i < num_light_vertices; i++)
        full_path[path_length - i] = light_path[i];
      real px, py;
      camera->get_pixel_coordinate(
          normalized(full_path[1].pos - full_path[0].pos), px, py);
      ...
      if (print_path_policy == "all" ||
        ...
        printf("Abnormal Path: #Eye %d, #Light %d", num_eye_vertices,
               num_light_vertices);
        printf("  f = %.10f %.10f %.10f, p = %.10f, c = %.10f, %.10f, %.10f\n",
               f[0], f[1], f[2], p, c[0], c[1], c[2]);
        for (int i = 0; i <= path_length; i++) {
          auto &v = full_path[i];
          printf("  pos = %f %f %f, normal = %f %f %f\n", v.pos[0], v.pos[1],
                 v.pos[2], v.normal[0], v.normal[1], v.normal[2]);
          ...
          if (i >= 1 && i < path_length) {
            ...
            printf("  #brdf = %s, pdf = %f, evaluate_bsdf = %f %f %f\n",
                   b.get_name().c_str(), p, brdf.x, brdf.y, brdf.z);
      ...
      if (print_path_policy != "none" &&
          (!f.is_normal() || abnormal(p) || !c.is_normal())) {
...
double BidirectionalRenderer::mis_weight(const Path &path,
                                         const int num_eye_vert_spec,
                                         const int num_light_vert_spec,
                                         bool including_connection,
                                         int merging_factor) {
  const double p_i =
      path_pdf(path, num_eye_vert_spec,
               num_light_vert_spec);  // TODO: not necessary re-evaluation
  const double p_all =
      path_total_pdf(path, including_connection, merging_factor);
...
Vector3d BidirectionalRenderer::path_throughput(const Path &path) {
  ...
  for (int i = 0; i < path.size(); i++) {
    if (i == 0) {
      ...
      const Vector3 in_dir = !path[i].connected
                                 ? path[i].in_dir
                                 : normalize(path[i - 1].pos - path[i].pos);
      const Vector3 out_dir = !path[i].connected
                                  ? path[i].out_dir
                                  : normalize(path[i + 1].pos - path[i].pos);
      if (path[i].bsdf.is_emissive() ||
...
double BidirectionalRenderer::geometry_term(const Vertex &current,
                                            const Vertex &next) {
...
double BidirectionalRenderer::direction_to_area(const Vertex &current,
                                                const Vertex &next) {
  const Vector3 v = next.pos - current.pos;
  const real dist2 = dot(v, v);
...
void BidirectionalRenderer::write_path_contribution(const PathContribution &pc,
                                                    const real scaling) {
