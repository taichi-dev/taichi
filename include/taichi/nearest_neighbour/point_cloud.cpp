...
void NearestNeighbour2D::initialize(const std::vector<Vector2> &data_points) {
  ...
  ann_kdtree = std::shared_ptr<ANNkd_tree>(
      new ANNkd_tree(ann_data_points, (int)data_points.size(), 2));
}
...
void NearestNeighbour2D::query_n(Vector2 p,
                                 int n,
...
void NearestNeighbour2D::query_n_index(Vector2 p,
                                       int n,
