...
bool Game::MouseButtonEvent(int button, int action) {
  if (designer.MouseButtonEvent(button, action))
    return true;
  if (bodyLinker.MouseButtonEvent(button, action))
    return true;
...
bool Game::KeyEvent(int key, int action) {
  if (designer.KeyEvent(key, action))
    return true;
  if (bodyLinker.KeyEvent(key, action))
    return true;
  if (ShapeFactory::KeyEvent(key, action))
    return true;

  if (action == GLFW_RELEASE)
    return false;
  if (key == GLFW_KEY_PAUSE)
    return settings.pause ^= 1, true;
  if (key == GLFW_KEY_F1)
    return Test1(), true;
  if (key == GLFW_KEY_F2)
    return Test2(), true;
...
void Game::Run() {
  ...
  for (int k = 0;; k++) {
    input.Update();
    {
      if (input.keyboard.NeedProcess((int)'X')) {
        if (physics.objects.size() > 1)
          while (physics.objects.size() > 1)
            physics.DeleteLastObject();
        else if (physics.objects.size() == 1)
          physics.DeleteLastObject();
      }
      //            if (input.keyboard.IsPressed(GLFW_LCTRL) &&
      //            input.mouse.IsPressed(GLFW_MOUSE_1)) {
      //                waterDrop
      //                physics.AddObject();
      //            }
    }
    ...
    if (target != NULL) {
      //            if (input.keyboard.NeedProcess('F'))
      //            target->SetFixed(!target->GetFixed());
      // if (input.keyboard.NeedProcess(GLFW_KEY_DEL) && target !=
      // physics.GetWorldBox()) {
