...
def test_mpm99_aot():
    ...
    grid_v_int = ti.Vector.field(
        2, dtype=int, shape=(n_grid, n_grid)
    )  # grid node momentum/velocity
    ...
    def substep():
        ...
        for p in x:  # Particle state update and scatter to grid (P2G)
            ...
            for i, j in ti.static(
                ...
                grid_v_int[base + offset] += int(
                    ti.floor(
                        0.5 + weight * (p_mass * v[p] + affine @ dpos) * (2.0**v_exp)
                    )
                )
        ...
        for i, j in grid_m:
            if grid_m_int[i, j] > 0:  # No need for epsilon here
                ...
                grid_v[i, j] = (2 ** (m_exp - v_exp) / grid_m_int[i, j]) * grid_v_int[
                    i, j
...
def test_sequential_dispatch():
    ...
    sym_args = ti.graph.Arg(
        ti.graph.ArgKind.MATRIX, "test_arg", ti.types.vector(3, ti.i32)
    )
