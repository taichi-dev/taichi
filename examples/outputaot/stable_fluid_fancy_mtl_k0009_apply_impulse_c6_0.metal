...
class mtl_k0009_apply_impulse_c6_0_args {
 public:
  explicit mtl_k0009_apply_impulse_c6_0_args(device byte* addr) : addr_(addr) {}
  device float* arg0() {
    // scalar, size=4 B
...
void mtl_k0009_apply_impulse_c6_0_0_func(
  ...
  mtl_k0009_apply_impulse_c6_0_args kernel_ctx_(ctx_addr);
  ...
  constexpr int32_t tmp10276 = 0;
  constexpr int32_t tmp10202 = 1023;
  constexpr int32_t tmp10198 = 511;
  constexpr float tmp36 = 1000.0;
  constexpr float tmp25 = 0.0023980816;
  constexpr float tmp21 = 0.5;
  const int tmp3 = linear_loop_idx_;
  constexpr int32_t tmp10164 = 10;
  const int32_t tmp10165 = (tmp3 >> tmp10164);
  const int32_t tmp10167 = (tmp10165 & tmp10198);
  const int32_t tmp10171 = (tmp3 & tmp10202);
  constexpr int32_t tmp13 = 417;
  const int32_t tmp14 = -(tmp10167 < tmp13);
  constexpr int32_t tmp16 = 597;
  const int32_t tmp17 = -(tmp10171 < tmp16);
  const int32_t tmp18 = (tmp14 & tmp17);
  if (tmp18) {
    const float tmp20 = static_cast<float>(tmp10167);
    const float tmp22 = (tmp20 + tmp21);
    const float tmp23 = static_cast<float>(tmp10171);
    ...
    S0 tmp10022(root_addr);
    S0_ch tmp10024 = tmp10022.children(tmp10276);
    S11 tmp10025 = tmp10024.get2(runtime_, mem_alloc_);
    const int32_t tmp10385 = (tmp10167 << tmp10164);
    const int32_t tmp10283 = (tmp10171 + tmp10385);
    S11_ch tmp10029 = tmp10025.children(tmp10283);
    device float* tmp10030 = tmp10029.get0(runtime_, mem_alloc_).val;
    const auto tmp44 = *tmp10030;
    device float* tmp10042 = tmp10029.get1(runtime_, mem_alloc_).val;
    const auto tmp46 = *tmp10042;
...
void mtl_k0009_apply_impulse_c6_0_1_func(
    device byte* root_addr,
    device byte* global_tmps_addr,
    device byte* ctx_addr,
    device byte* runtime_addr,
  ...
  AssertRecorder assert_rec_(print_assert_addr);
  device auto* print_alloc_ = reinterpret_cast<device PrintMsgAllocator*>(print_assert_addr + 300);
  constexpr int32_t tmp10308 = 0;
  constexpr int32_t tmp10274 = 4095;
  constexpr int32_t tmp10270 = 2047;
  constexpr float tmp87 = 1000.0;
  constexpr float tmp76 = 0.0005995204;
  constexpr float tmp72 = 0.5;
  const int tmp54 = linear_loop_idx_;
  constexpr int32_t tmp10204 = 12;
  const int32_t tmp10205 = (tmp54 >> tmp10204);
  const int32_t tmp10207 = (tmp10205 & tmp10270);
  const int32_t tmp10211 = (tmp54 & tmp10274);
  constexpr int32_t tmp64 = 1668;
  const int32_t tmp65 = -(tmp10207 < tmp64);
  constexpr int32_t tmp67 = 2388;
  const int32_t tmp68 = -(tmp10211 < tmp67);
  const int32_t tmp69 = (tmp65 & tmp68);
  if (tmp69) {
    const float tmp71 = static_cast<float>(tmp10207);
    const float tmp73 = (tmp71 + tmp72);
    const float tmp74 = static_cast<float>(tmp10211);
    ...
    S0 tmp10070(root_addr);
    S0_ch tmp10072 = tmp10070.children(tmp10308);
    S1 tmp10073 = tmp10072.get0(runtime_, mem_alloc_);
    const int32_t tmp10387 = (tmp10207 << tmp10204);
    const int32_t tmp10315 = (tmp10211 + tmp10387);
    S1_ch tmp10077 = tmp10073.children(tmp10315);
    device float* tmp10078 = tmp10077.get0(runtime_, mem_alloc_).val;
    const auto tmp98 = *tmp10078;
    device float* tmp10090 = tmp10077.get1(runtime_, mem_alloc_).val;
    const auto tmp102 = *tmp10090;
    device float* tmp10102 = tmp10077.get2(runtime_, mem_alloc_).val;
    const auto tmp106 = *tmp10102;
    device float* tmp10114 = tmp10077.get3(runtime_, mem_alloc_).val;
    const auto tmp110 = *tmp10114;
...
kernel void mtl_k0009_apply_impulse_c6_0_0(
  ...
  for (int ii = begin_; ii < end_; ii += ugrid_size_) {
    mtl_k0009_apply_impulse_c6_0_0_func(root_addr, global_tmps_addr, ctx_addr, runtime_addr, print_assert_addr, ii);
  }
}

kernel void mtl_k0009_apply_impulse_c6_0_1(
    device byte* root_addr [[buffer(0)]],
    device byte* global_tmps_addr [[buffer(1)]],
    device byte* ctx_addr [[buffer(2)]],
    device byte* runtime_addr [[buffer(3)]],
  ...
  device auto *mem_alloc_ = reinterpret_cast<device MemoryAllocator *>(runtime_ + 1);
  for (int ii = begin_; ii < end_; ii += ugrid_size_) {
    mtl_k0009_apply_impulse_c6_0_1_func(root_addr, global_tmps_addr, ctx_addr, runtime_addr, print_assert_addr, ii);
  }
}
