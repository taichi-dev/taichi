...
std::string convert(std::string new_name) {
	// Evil C++ mangling on Windows will lead to "unsupported characters in symbol" error in LLVM PTX printer.
	// Convert here.
    for (int i = 0; i < (int)new_name.size(); i++) {
      ...
      TI_ASSERT(std::isalpha(new_name[i]) || std::isdigit(new_name[i]) || new_name[i] == '_' || new_name[i] == '.');
    }
    TI_ASSERT(isalpha(new_name[0]) || new_name[0] == '_');
    return new_name;
}
