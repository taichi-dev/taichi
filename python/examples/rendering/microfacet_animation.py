...
def create_scene(t):
    ...
    camera = tc.Camera('pinhole', width=width, height=height, fov=50,
                       origin=(0, 2, 10), look_at=(0, -0.5, 0), up=(0, 1, 0))
    ...
    with scene:
        ...
        mesh = tc.Mesh('plane',
                       tc.SurfaceMaterial('microfacet', color=(1, 1, 1), roughness=roughness, f0=1),
                       translate=(0, 0, 0), scale=7, rotation=(0, 0, 0))
        scene.add_mesh(mesh)
        ...
        for i in range(5):
            with tc.transform_scope(translate=(1.4 * (i - 2), 0.6, 0)):
                with tc.transform_scope(scale=(0.3, 1, 0.5), rotation=(90, 0, 0)):
                    mesh = tc.Mesh('plane',
                                   tc.SurfaceMaterial('emissive', color=colorsys.hls_to_rgb(i * 0.2, 0.5, 1.0)))
                    scene.add_mesh(mesh)
...
if __name__ == '__main__':
    ...
    for i in range(frames + 1):
        ...
        renderer.initialize(preset='pt', scene=create_scene(0.5 * (1 - math.cos(math.pi * i / frames))))
