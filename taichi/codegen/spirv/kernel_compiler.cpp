...
std::unique_ptr<lang::IRNode> KernelCompiler::compile(
    const CompileConfig &compile_config,
    const Kernel &kernel_def) const {
  // FIXME: impl clone for frontend IR
  // auto ir = irpass::analysis::clone(kernel_def.ir.get());
  auto ir = std::move(const_cast<Kernel &>(kernel_def).ir);
...
std::unique_ptr<lang::CompiledKernelData> KernelCompiler::compile(
  ...
  codegen.run(internal_data.kernel_attribs, internal_data.spirv_src);
  internal_data.num_snode_trees = config_.compiled_struct_data->size();
  return std::make_unique<spirv::CompiledKernelData>(internal_data);
}
