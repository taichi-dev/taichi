...
//#####################################################################
// Subroutine SPGrid_Computations::Add
//#####################################################################
#ifndef __SPGrid_Add_h__
#define __SPGrid_Add_h__
...
Add(SPGrid_Allocator<T_STRUCT,d>& allocator,const std::pair<const unsigned long*,unsigned>& blocks,
    T_FIELD T_STRUCT::* x_field,T_FIELD T_STRUCT::* y_field,T_FIELD T_STRUCT::* destination_field)
{
    ...
    for(SPGrid_Block_Iterator<typename Data_array_type::MASK> iterator(blocks);iterator.Valid();iterator.Next())
        iterator.Data(destination)=iterator.Data(x)+iterator.Data(y);
}
...
Masked_Add(SPGrid_Allocator<T_STRUCT,d>& allocator,const std::pair<const unsigned long*,unsigned>& blocks,
    T_DATA T_STRUCT::* x_field,T_DATA T_STRUCT::* y_field,T_DATA T_STRUCT::* destination_field,
