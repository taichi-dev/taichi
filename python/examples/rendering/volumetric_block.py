...
def create_scene():
    ...
    camera = Camera('pinhole', width=width, height=height, fov=30,
                    origin=(4, 0, 15), look_at=(0, 0, 0), up=(0, 1, 0))
    ...
    with scene:
        ...
        mesh = tc.Mesh('plane', tc.SurfaceMaterial('emissive', color=(emission, emission, emission)),
                       translate=(300, 200, 300), scale=30, rotation=(-90, 0, 0))
        scene.add_mesh(mesh)

        material = SurfaceMaterial('diffuse', color=(0.5, 1, 1), roughness=1.0, f0=1)
        scene.add_mesh(Mesh('cube', material=material, translate=(0, 0, -2.0), scale=(1, 1, 1)))

        material = SurfaceMaterial('diffuse', color=(1, 0.5, 1), roughness=1.0, f0=1)
        scene.add_mesh(Mesh('cube', material=material, translate=(0, -2.0, 0), scale=(1, 1, 1)))

        material = SurfaceMaterial('diffuse', color=(1, 1, 0.5), roughness=1.0, f0=1)
        scene.add_mesh(Mesh('cube', material=material, translate=(-2.0, 0, 0), scale=(1, 1, 1)))
...
def render():
    ...
    renderer.set_post_processor(LDRDisplay(exposure=0.9, bloom_radius=0.0, bloom_threshold=1.0))
