...
class TestStmtVector : public Stmt {
 ...
 public:
  TestStmtVector(const std::vector<Stmt *> &vec1,
      const std::vector<Stmt *> &vec2) : vec1(vec1), vec2(vec2) {
    TI_STMT_REG_FIELDS;
  }
...
TI_TEST("test_stmt_field_manager_with_vector") {
  ...
  auto a = Stmt::make<TestStmtVector>(
      std::vector<Stmt *>(), std::vector<Stmt *>(1, one.get()));

  TI_CHECK(a->num_operands() == 1);
  TI_CHECK(a->field_manager.fields.size() == 2);

  auto b = Stmt::make<TestStmtVector>(
      std::vector<Stmt *>(), std::vector<Stmt *>(1, one.get()));

  TI_CHECK(a->field_manager.equal(b->field_manager) == true);

  auto c = Stmt::make<TestStmtVector>(
      std::vector<Stmt *>(1, one.get()), std::vector<Stmt *>());
