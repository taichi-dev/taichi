...
def compile_numerical_aot_test(arch):
    ...
    @ti.kernel
    def fill_matrix_array_with_fp16(arr: ti.types.ndarray(
        dtype=ti.types.matrix(n=2, m=3, dtype=ti.f16), ndim=1), val: ti.f16):
        for i in arr:
            arr[i] = [[val, val, val], [val, val, val]]

    @ti.kernel
    def compute_kernel(
            pose: ti.types.ndarray(dtype=ti.types.matrix(3, 4, dtype=ti.f16),
                                   ndim=0),
            directions: ti.types.ndarray(dtype=ti.types.matrix(1,
                                                               3,
                                                               dtype=ti.f16),
                                         ndim=1),
            out_0: ti.types.ndarray(dtype=ti.types.vector(n=3, dtype=ti.f16),
                                    ndim=1),
            out_1: ti.types.ndarray(dtype=ti.types.vector(n=3, dtype=ti.f16),
                                    ndim=1)):
