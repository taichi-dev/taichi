...
The function `ti.sqrt` (and those who rely on it, such as `ti.norm`) may have a lower precision on the CUDA backend than on the CPU backends, see this [issue](https://github.com/taichi-dev/taichi/issues/6346). To work around this problem, you can set the default precision to `ti.f64` and turn off the `fast_math` option:
