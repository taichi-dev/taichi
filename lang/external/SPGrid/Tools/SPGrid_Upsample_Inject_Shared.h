...
//#####################################################################
// Subroutine SPGrid_Computations::Upsample_Inject_Shared
//#####################################################################
#ifndef __SPGrid_Upsample_Inject_Shared_h__
#define __SPGrid_Upsample_Inject_Shared_h__

namespace SPGrid_Computations{

template<class T_STRUCT,class T_DATA,class T_FLAGS,int d> void
Upsample_Inject_Shared(SPGrid_Allocator<T_STRUCT,d>& allocator,SPGrid_Allocator<T_STRUCT,d>& coarse_allocator,
    const std::pair<const unsigned long*,unsigned>& blocks,T_DATA T_STRUCT::* data_field,
    T_FLAGS T_STRUCT::* flags_field,const T_FLAGS mask)
{
    ...
    Const_data_array_type coarse_data=coarse_allocator.Get_Const_Array(data_field);
    ...
    for(SPGrid_Block_Iterator<typename Data_array_type::MASK> iterator(blocks);iterator.Valid();iterator.Next())
        if(iterator.Data(flags) & mask){
            ...
            iterator.Data(data)=coarse_data(coarse_offset);}
}
