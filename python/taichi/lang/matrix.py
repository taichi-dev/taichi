    ...
    def from_kernel_struct_ret(self, launch_ctx, ret_index=()):
        if self.dtype in primitive_types.integer_types:
            if is_signed(cook_dtype(self.dtype)):
                get_ret_func = _ti_ccore.tie_LaunchContextBuilder_get_struct_ret_int
            else:
                get_ret_func = _ti_ccore.tie_LaunchContextBuilder_get_struct_ret_uint
        elif self.dtype in primitive_types.real_types:
            get_ret_func = _ti_ccore.tie_LaunchContextBuilder_get_struct_ret_float
        else:
            raise TaichiRuntimeTypeError(f"Invalid return type on index={ret_index}")
        return self([get_ret_func(launch_ctx, ret_index + (i,)) for i in range(self.m * self.n)])
