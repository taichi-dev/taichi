...
@ti.kernel
def generate_laplacian_matrix(A: ti.sparse_matrix_builder()):
...
@ti.kernel
def copy_divergence(x_in: ti.template(), x_out: ti.template()):
    for I in ti.grouped(x_in):
        x_out[I[0] * res + I[1]] = -x_in[I]


@ti.kernel
def assign_pressures(x_in: ti.ext_arr(), x_out: ti.template()):
    for I in ti.grouped(x_out):
        x_out[I] = x_in[I[0] * res + I[1]]
