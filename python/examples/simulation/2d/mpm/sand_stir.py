...
if __name__ == '__main__':
    res = tuple([256] * 2)
    simulator = MPMSimulator(res=res, simulation_time=10, frame_dt=5e-2, base_delta_t=2e-3)
    ...
    def levelset_generator(t):
        ...
        levelset.add_sphere(Vector(0.5 + 0.25 * math.cos(t * velocity), 0.5 + 0.25 * math.sin(t * velocity)), 0.1,
                            False)
        levelset.add_sphere(
            Vector(0.5 + 0.25 * math.cos(t * velocity + math.pi), 0.5 + 0.25 * math.sin(t * velocity + math.pi)), 0.1,
            False)
