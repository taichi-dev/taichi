...
class Physics {
 ...
 public:
  ...
  void AddForce(Force *force) { forces.push_back(force); }
  ...
  Object *GetWorldBox() { return worldBox; }
  void DeleteLastObject() { DeleteObject(objects.back()); }
  void DeleteAllObjects() {
    while ((int)objects.size())
      DeleteLastObject();
  }
    ...
    while (flg) {
      ...
      for (int i = 0; i < (int)objects.size(); i++) {
        if ((objects[i]->position -
             Vector2D(screenWidth / 2, screenHeight / 2, 1))
                .GetLength() > 20000) {
          flg = true;
  ...
  void SelectObject(Vector2D p, Object *&object, Vector2D &r) {
    object = NULL;
    for (int i = 0; i < (int)objects.size(); i++)
      if (objects[i]->IsPointInside(p) &&
          (object == NULL || objects[i]->priority > object->priority)) {
        object = objects[i];
  ...
  void Redraw() {
    ...
    for (int i = 0; i < (int)contactPoints.size(); i++)
        graphics.DrawPoint(contactPoints[i].x, contactPoints[i].y,
    Colors::Black, 3);
    */
  }
