...
def create_taichi_scene(eye_position):
  ...
  with scene:
    ...
    vol = VolumeMaterial(
        'voxel', scattering=100, absorption=0, resolution=256, tex=tex)
...
def create_mpm_snow_block(fn):
  ...
  transform = tc_core.Matrix4(1.0).scale_s(2).scale(
      Vector(2.0, 0.5, 1.0)).translate(Vector(-2, -0.99, -1))
  vol = VolumeMaterial(
      'sdf_voxel',
      scattering=5,
      absorption=0,
      tex=tex,
      resolution=(511, 127, 255),
...
def render_snow_frame(frame):
  ...
  renderer.initialize(
      preset='pt', scene=create_snow_scene(frame), sampler='prand')
