...
def fetch_active_contributors():
    ...
    contributors_json = requests.get(
        f'https://api.github.com/repos/taichi-dev/taichi/contributors?per_page={per_page}'
    ).json()
    contributors = []
    ...
    assert len(
        contributors
    ) < per_page, 'Please extend the program to handle >= 100 contributors'
    ...
    while not eof:
        ...
        commits_json = requests.get(
            f'https://api.github.com/repos/taichi-dev/taichi/commits?per_page={per_page}&page={page}'
        ).json()
