...
void mtl_k0009_apply_impulse_c6_0_0_func(
  ...
  constexpr int32_t tmp9724 = 0;
  constexpr int32_t tmp9666 = 1023;
  constexpr int32_t tmp9662 = 511;
  constexpr float tmp36 = 1000.0;
  constexpr float tmp25 = 0.0023980816;
  constexpr float tmp21 = 0.5;
  const int tmp3 = linear_loop_idx_;
  constexpr int32_t tmp9628 = 10;
  const int32_t tmp9629 = (tmp3 >> tmp9628);
  const int32_t tmp9631 = (tmp9629 & tmp9662);
  const int32_t tmp9635 = (tmp3 & tmp9666);
  constexpr int32_t tmp13 = 417;
  const int32_t tmp14 = -(tmp9631 < tmp13);
  constexpr int32_t tmp16 = 597;
  const int32_t tmp17 = -(tmp9635 < tmp16);
  const int32_t tmp18 = (tmp14 & tmp17);
  if (tmp18) {
    const float tmp20 = static_cast<float>(tmp9631);
    const float tmp22 = (tmp20 + tmp21);
    const float tmp23 = static_cast<float>(tmp9635);
    ...
    S0 tmp9510(root_addr);
    S0_ch tmp9512 = tmp9510.children(tmp9724);
    S9 tmp9513 = tmp9512.get2(runtime_, mem_alloc_);
    const int32_t tmp9817 = (tmp9631 << tmp9628);
    const int32_t tmp9731 = (tmp9635 + tmp9817);
    S9_ch tmp9517 = tmp9513.children(tmp9731);
    device float* tmp9518 = tmp9517.get0(runtime_, mem_alloc_).val;
    const auto tmp44 = *tmp9518;
    device float* tmp9530 = tmp9517.get1(runtime_, mem_alloc_).val;
    const auto tmp46 = *tmp9530;
...
void mtl_k0009_apply_impulse_c6_0_1_func(
  ...
  AssertRecorder assert_rec_(print_assert_addr);
  device auto* print_alloc_ = reinterpret_cast<device PrintMsgAllocator*>(print_assert_addr + 300);
  constexpr int32_t tmp9756 = 0;
  constexpr int32_t tmp9722 = 4095;
  constexpr int32_t tmp9718 = 2047;
  constexpr float tmp87 = 1000.0;
  constexpr float tmp76 = 0.0005995204;
  constexpr float tmp72 = 0.5;
  const int tmp54 = linear_loop_idx_;
  constexpr int32_t tmp9668 = 12;
  const int32_t tmp9669 = (tmp54 >> tmp9668);
  const int32_t tmp9671 = (tmp9669 & tmp9718);
  const int32_t tmp9675 = (tmp54 & tmp9722);
  constexpr int32_t tmp64 = 1668;
  const int32_t tmp65 = -(tmp9671 < tmp64);
  constexpr int32_t tmp67 = 2388;
  const int32_t tmp68 = -(tmp9675 < tmp67);
  const int32_t tmp69 = (tmp65 & tmp68);
  if (tmp69) {
    const float tmp71 = static_cast<float>(tmp9671);
    const float tmp73 = (tmp71 + tmp72);
    const float tmp74 = static_cast<float>(tmp9675);
    ...
    S0 tmp9558(root_addr);
    S0_ch tmp9560 = tmp9558.children(tmp9756);
    S1 tmp9561 = tmp9560.get0(runtime_, mem_alloc_);
    const int32_t tmp9819 = (tmp9671 << tmp9668);
    const int32_t tmp9763 = (tmp9675 + tmp9819);
    S1_ch tmp9565 = tmp9561.children(tmp9763);
    device float* tmp9566 = tmp9565.get0(runtime_, mem_alloc_).val;
    const auto tmp98 = *tmp9566;
    device float* tmp9578 = tmp9565.get1(runtime_, mem_alloc_).val;
    const auto tmp102 = *tmp9578;
    device float* tmp9590 = tmp9565.get2(runtime_, mem_alloc_).val;
    const auto tmp106 = *tmp9590;
    const float tmp109 = (tmp98 + tmp91);
    const float tmp112 = (tmp102 + tmp93);
    const float tmp115 = (tmp106 + tmp95);
    *tmp9566 = tmp109;
    *tmp9578 = tmp112;
    *tmp9590 = tmp115;
  } else {
