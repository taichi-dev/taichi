...
public:
	...
	Array2D<float> mass;
	Array2D<int> id;
	vector<ivec2> id_to_pos;
	vector<ivec2> z_to_xy;
	int width, height;
	int valid_count;
	bool need_system;
	void initialize(int width, int height, bool need_system) {
		...
		if (need_system)
			_system = vector<mat2>((width)* (height)* SYSTEM_STRIDE_PER_GRID);
		id_to_pos = vector<ivec2>(width * height);
		z_to_xy = vector<ivec2>(width * height);
		initialize_z_order();
	}
