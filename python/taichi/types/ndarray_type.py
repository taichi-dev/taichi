...
class NdarrayType:
    ...
    def check_matched(self, ndarray_type: NdarrayTypeMetadata):
        if self.element_dim is not None and self.element_dim > 0:
            ...
            if self.element_dim != len(
                    ndarray_type.element_type.element_shape()):
                raise ValueError(
                    f"Invalid argument into ti.types.ndarray() - required element_dim={self.element_dim}, but {len(ndarray_type.element_type.element_shape())} is provided"
                )
        ...
        if self.element_shape is not None and len(self.element_shape) > 0:
            ...
            if self.element_shape != ndarray_type.element_type.element_shape():
                raise ValueError(
                    f"Invalid argument into ti.types.ndarray() - required element_shape={self.element_shape}, but {ndarray_type.element_type.element_shape()} is provided"
                )
