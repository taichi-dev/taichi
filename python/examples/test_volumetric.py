...
def create_mis_scene(eye_position):
    ...
    camera = Camera('perspective', width=width, height=height, fov_angle=70,
                    origin=(0, 2, 3), look_at=(0, 0, 0), up=(0, 1, 0))
    ...
    with scene:
        ...
        vol = VolumeMaterial("homogeneous", scattering=1, absorption=0)
        material.set_internal_material(vol)
        mesh = Mesh('cube', material=material,
                    translate=(0, -0, -0), scale=(0.2, 0.7, 0.2), rotation=(0, 0, 0))
        scene.add_mesh(mesh)
...
if __name__ == '__main__':
    ...
    renderer.initialize(min_path_length=1, max_path_length=10,
                        ...
                        num_threads=8)
    renderer.set_post_processor(LDRDisplay(exposure=0.1, bloom_radius=0.00))
