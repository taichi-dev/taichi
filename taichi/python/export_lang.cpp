...
#undef PER_EXTENSION
  ...
  m.def("expr_atomic_cas", [&](const Expr &a, const Expr &b, const Expr &c) {
    return Expr::make<AtomicOpExpression>(AtomicOpType::cas, ptr_if_global(a),
                                          load_if_ptr(b), load_if_ptr(c));
  });
