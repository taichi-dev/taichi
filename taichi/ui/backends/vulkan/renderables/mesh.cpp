...
void Mesh::update_data(const MeshInfo &info, const Scene &scene) {
  ...
  if (info.mesh_attribute_info.has_attribute) {
    ...
    {
      TI_ERROR("Data Type transforms of Matrix Field must be ti.f32 or ti.u32 or ti.i32");
    }

    size_t correct_mesh_ssbo_size = num_instances_ * attr_field.matrix_rows * attr_field.matrix_cols * sizeof(float);
  ...
  if (info.mesh_attribute_info.has_attribute) {
    ...
    DevicePtr attr_dev_ptr = info.mesh_attribute_info.mesh_attribute.dev_alloc.get_ptr();
    if (prog) {
      attr_dev_ptr = get_device_ptr(prog, info.mesh_attribute_info.mesh_attribute.snode);
    }
    ...
    if (memcpy_cap == Device::MemcpyCapability::Direct) {
      Device::memcpy_direct(mesh_storage_buffer_.get_ptr(), attr_dev_ptr, mesh_ssbo_size_);
...
void Mesh::record_this_frame_commands(
  ...
  if (indexed_) {
    command_list->draw_indexed_instance(config_.draw_index_count,
                               num_instances_,
                               config_.draw_first_vertex,
                               config_.draw_first_index,
                               start_instance_);
  } else {
    command_list->draw_instance(config_.draw_vertex_count, num_instances_, config_.draw_first_vertex, start_instance_);
  }
}
