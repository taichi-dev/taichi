...
cpp_tests_path = os.path.join(curr_dir, '/build/taichi_cpp_tests')
c_api_tests_path = os.path.join(curr_dir, '/build/taichi_c_api_tests')
...
def init_dict(run_dict, aot_files):
    ...
    for cpp_test_name, value in test_config["aot_test_cases"].items():
        ...
        run_dict[value[0][3][:-3]].append(cpp_tests_path + " --gtest_filter=" +
                                          cpp_test_name)

    for cpp_test_name, value in test_config["capi_aot_test_cases"].items():
        if value[1] != "--arch=vulkan": continue
        run_dict[value[0][3][:-3]].append(c_api_tests_path +
                                          " --gtest_filter=" + cpp_test_name)
...
def generate():
    ...
    for x in aot_files:
        ...
        os.environ[
            "TAICHI_AOT_FOLDER_PATH"] = 'tests/cpp/aot/python_scripts/' + path_name
        try:
