...
std::tuple<Expr, Expr, Expr> eig_3x3_export(ASTBuilder *ast_builder,
  ...
  ast_builder->insert_assignment(c1, A00 * A11 + A00 * A22 + A11 * A22 - (dd + ee + ff));
  ast_builder->insert_assignment(c0, A22 * dd + A00 * ee + A11 * ff - A00 * A11 * A22 - Expr(Tf(2.0)) * A02 * de);
  ast_builder->insert_assignment(p, m * m - Expr(Tf(3.0)) * c1);
  ast_builder->insert_assignment(q, m * (p - Expr(Tf(1.5)) * c1) - Expr(Tf(13.5)) * c0);
  ast_builder->insert_assignment(sqrt_p, sqrt(abs(p)));
  ast_builder->insert_assignment(phi, Expr(Tf(27.0)) * (Expr(Tf(0.25)) * c1 * c1 * (p - c1) + c0 * (q + Expr(Tf(6.75)) * c0)));
  ast_builder->insert_assignment(phi, Expr(Tf(1.0))/ Expr(Tf(3.0)) * atan2(sqrt(abs(phi)), q));
  ast_builder->insert_assignment(c, sqrt_p * cos(phi));
  ast_builder->insert_assignment(s, Expr(Tf(1.0)) / Expr(M_SQRT3) * sqrt_p * sin(phi));
