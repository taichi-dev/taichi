  ...
  with scene:
    ...
    scene.add_mesh(
        tc.Mesh(
            'plane',
            tc.SurfaceMaterial(
                'pbr', diffuse_map=gradient, specular_map=1 - gradient),
            scale=(10, 1, 1),
            translate=(0, 0.3, 2),
            rotation=(0, 0, 0)))
    ...
    scene.add_mesh(
        tc.Mesh(
            'plane',
            tc.SurfaceMaterial(
                'glossy', color=(1, 1, 1), glossiness_map=gradient * 100),
            scale=(10, 1, 1),
            translate=(0, 0.3, 4.5),
            rotation=(0, 0, 0)))
    ...
    scene.add_mesh(
        tc.Mesh(
            ...
            tc.SurfaceMaterial(
                'pbr',
                diffuse_map=gradient * (1, 0, 0) + (1 - gradient) * (0, 1, 1)),
            scale=(10, 1, 1),
            translate=(0, 0.3, 7),
            rotation=(0, 0, 0)))
    ...
    envmap = tc.EnvironmentMap(
        'base',
        filepath=tc.settings.get_asset_path('envmaps/schoenbrunn-front_hd.hdr'))
...
if __name__ == '__main__':
  ...
  renderer.initialize(
      preset='pt', scene=create_scene(), luminance_clamping=1000)
