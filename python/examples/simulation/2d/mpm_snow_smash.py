...
if __name__ == '__main__':
    ...
    simulator = create_mpm_simulator(resolution, 15, frame_dt=2e-2, base_delta_t=1e-6, async=True, maximum_delta_t=2e-2)

    simulator.add_event(-1, lambda s: s.add_particles_sphere(Vector(0.32, 0.45), 0.10, 'ep', compression=0.8,
                                                             velocity=Vector(1.0, 0.0), theta_s=0.002))
    simulator.add_event(-1, lambda s: s.add_particles_sphere(Vector(1.32, 0.60), 0.10, 'ep', compression=0.8,
                                                             velocity=Vector(-1.0, 0.0), theta_s=0.002))
