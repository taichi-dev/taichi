...
from bokeh.models import ColumnDataSource, FactorRange, Select, CheckboxGroup, Div, DataTable, TableColumn
from bokeh.plotting import curdoc, figure, show
from bokeh.transform import factor_cmap

from misc import toolbox, html_str_format, customize_palette
...
for case in cases_list:
    item_dict[case] = benchmark_results['microbenchmarks']['cuda'][case]['items']
# metrics
metric_dict = {}
for case in cases_list:
    metric_dict[case] = benchmark_results['microbenchmarks']['cuda'][case]['items']['metrics']
        ...
        for i in range(len(tags.labels)):
            if i in tags.active:
                ...
                for case in case_list:
                    if (tags.labels[i] in temp_dict[case]['tags']) and (case not in remove_list):
...
def create_items():
    ...
    for item in item_list:
        ...
        if item == 'metrics':
            tgas_active = [0]
        tgas_group = CheckboxGroup(labels=tags, active=tgas_active, inline = True)
...
def create_figure(grouped_by_item, filterd_data):
    ...
    source = ColumnDataSource(data=dict(grouped_tag_name=grouped_tag_name, grouped_result=grouped_result))
    ...
    if 1 in config_checkbox.active: # show_logaxis_y
        y_axis_config = {'y_axis_type': 'log', 'y_range': (min(grouped_result)/2, max(grouped_result))}
    
    p = figure(width=1440, height=720, title=case_selection.value,
               x_range=FactorRange(*grouped_tag_name), 
               tooltips=[("case", "@grouped_tag_name"), ("time_ms", "@grouped_result")],
               toolbar_location="above", tools=toolbox,
               **y_axis_config)

    # bar color
    color_agrs = {}
    if 2 in config_checkbox.active: #show_colored_bar
        index_cmap = factor_cmap('grouped_tag_name', palette=customize_palette, factors=active_tags, end=1)
        color_agrs = {'fill_color': index_cmap}

    p.vbar(x='grouped_tag_name', top='grouped_result', bottom=min(grouped_result)/2, width=1, source=source, 
           line_color="white", **color_agrs)
...
def creat_table(filterd_data):
    ...
    data_table = DataTable(source=source, columns=columns, editable=True, width=1440,
                        index_position=0, index_header="row index", index_width=60)
    
    return data_table

def create_group_by_item_selection():
    item_list = list(item_dict[case_selection.value].keys())
    grouped_by_item_selection = Select(title='grouped by item:', value=item_list[0], options=item_list)
...
def update_options(attr, old, new):
    ...
    layout.children[0].children[1].children[0] = create_group_by_item_selection() # column2_item
    layout.children[0].children[1].children[1] = create_items()                   # column2_item

def update_figure(attr, old, new):
    case_data = benchmark_results['microbenchmarks']['cuda'][case_selection.value]['results']
    filterd_data = filtering_data(case_data, layout.children[0].children[1].children[1])
...
case_selection = Select(title='select case: ', value=cases_list[0], options=cases_list)
case_selection.on_change('value', update)
case_controls = column(case_selection, width=200)
# misc
config_checkbox = CheckboxGroup(labels=['show_label_x', 'show_logaxis_y', 'colored_bar'], active=[2])
...
case_data = benchmark_results['microbenchmarks']['cuda'][cases_list[0]]['results']
filterd_data = filtering_data(case_data, column2_item.children[1])
layout = column(row1_options, create_figure(grouped_by_item_selection.value, filterd_data), creat_table(filterd_data))
