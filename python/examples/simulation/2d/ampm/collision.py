...
if __name__ == '__main__':
    ...
    if async:
        ...
        simulator = MPMSimulator(res=res, simulation_time=0.3, frame_dt=frame_dt, base_delta_t=5e-5, async=False,
    ...
    def event(s):
        ...
        def get_polygon(center_x, center_y):
            ...
            return polygon
        s.add_particles_polygon(get_polygon(0.7, 0.53), 'ep', compression=0.7, velocity=Vector(-3, 0), theta_s=1, theta_c=1)
        s.add_particles_polygon(get_polygon(0.3, 0.47), 'ep', compression=0.7, velocity=Vector(3, 0), theta_s=1, theta_c=1)
