...
gravitation_position = [[0.3, 0.3], [0.3, 0.7], [0.7, 0.7], [0.7, 0.3]]
  ...
  for t in range(1, steps):
    ...
    if (t + 1) % interval == 0 and visualize:
      ...
      for i in range(n_gravitation):
        g = gravitation[t, i]
        g = int(g + 1 / 2 * 255)
        canvas.circle(tc.Vector(*gravitation_position[i])).radius(10).color(0x010101 * g).finish()

      canvas.circle(tc.Vector(goal[None][0], goal[None][1])).radius(10).color(0x3344cc).finish()
      canvas.circle(tc.Vector(x[t][0], x[t][1])).radius(10).color(0xF20530).finish()
...
def optimize():
  x[0] = [random.random(), random.random()]
  goal[None] = [random.random(), random.random()]
