...
This line calls the `ti.init` function to initialize some environment variables. The `init` function accepts several arguments to allow users to customize the runtime program. For now, we only introduce the most important one, namely the `arch`.
