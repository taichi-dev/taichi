...
bool Designer::MouseButtonEvent(int button, int action) {
  ...
  if (showGrid) {
      double step = 1;
      while (game->scoper.GetDistanceOnScr(Vector2D(0, 0, 1), Vector2D(0, step *
  2, 1)) < 30) step *= 2;
      ...
      if (action == GLFW_PRESS) {
          switch (type) {
              ...
              case 9:
                  ...
                  break;
              case 1:
                  if (points.size() && game->scoper.GetDistanceOnScr(p,
  points[0]) <= 5) {
                      Object *object =
  ShapeFactory::GeneratePolygonObject(points);
          ...
          switch (type) {
              ...
              case 2:
                  ...
                  object =
  ShapeFactory::GeneratePolygonObject(ShapeFactory::GetBoxPoints(center, p));
              ...
              case 3:
                  if (r.GetLength() == 0) return true;
                  object = ShapeFactory::GenerateGearObject(center,
  r.GetLength(), Polygon::GearD / r.GetLength());
                  object->SetPosition(center);
                  game->physics.AddConstantConstraint(new
  DistanceConstraint(game->physics.GetWorldBox(), center.GetVector(), object,
  Vector2D(0, 0, 0)));
                  break;
          }
          if (object != NULL)
...
void Designer::Redraw() {
  ...
  color, 2.0);
      ...
      case 2:
          if (input->mouse.IsPressed(GLFW_MOUSE_BUTTON_1)) {
              graphics.DrawLinesLoop(ShapeFactory::GetBoxPoints(center, p),
  Colors::White, 2.0);
          }
          break;
      case 4:
      ...
      case 3:
          if (input->mouse.IsPressed(GLFW_MOUSE_BUTTON_1)) {
              Vector2D r = p - center;
              vector<Vector2D> points = Polygon::GenerateGearPoints(center,
  r.GetLength(), Polygon::GearD / r.GetLength());
              graphics.DrawLinesLoop(points, Colors::White, 2.0);
          }
          break;
