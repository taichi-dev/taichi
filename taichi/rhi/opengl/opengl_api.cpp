...
bool initialize_opengl(bool use_gles, bool error_tolerance, bool reset) {
  ...
  if (supported[idx].has_value()) {  // this function has been called before
    if (supported[idx].value()) {    // detected to be true in last call
      return true;
    } else {
...
#undef PER_OPENGL_EXTENSION
  ...
  if (!use_gles && !opengl_extension_GL_ARB_compute_shader) {
    if (error_tolerance) {
      ...
      supported[idx] = std::make_optional<bool>(false);
      return false;
    }
  ...
  supported[idx] = std::make_optional<bool>(true);
  kUseGles = use_gles;
  return true;
}
