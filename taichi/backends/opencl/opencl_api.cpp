...
std::vector<cl_device_id> get_opencl_devices(cl_platform_id platform) {
  ...
  clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, devices.size(), devices.data(),
                 NULL);
  return devices;
}
...
void show_platforms_info_yaml(std::vector<cl_platform_id> platforms) {
  ...
  for (auto pl : platforms) {
    std::cout << "- name: " << get_platform_info(pl, CL_PLATFORM_NAME)
              << std::endl;
    std::cout << "  vendor: " << get_platform_info(pl, CL_PLATFORM_VENDOR)
              << std::endl;
    std::cout << "  version: " << get_platform_info(pl, CL_PLATFORM_VERSION)
              << std::endl;

    auto devices = get_opencl_devices(platforms[0]);
    std::cout << "  devices:" << std::endl;
    for (auto de : devices) {
      std::cout << "  - name: " << get_device_info(de, CL_DEVICE_NAME)
                << std::endl;
      auto type = get_device_info<cl_device_type>(de, CL_DEVICE_TYPE);
      std::cout << "    device_type: "
                << (type == CL_DEVICE_TYPE_CPU
                        ? "CPU"
                        : type == CL_DEVICE_TYPE_GPU ? "GPU" : "DEFAULT")
                << std::endl;
      std::cout << "    vendor: " << get_device_info(de, CL_DEVICE_VENDOR)
                << std::endl;
      std::cout << "    version: " << get_device_info(de, CL_DEVICE_VERSION)
                << std::endl;
      std::cout << "    address_bits: "
                << get_device_info<int>(de, CL_DEVICE_ADDRESS_BITS)
                << std::endl;
      std::cout << "    local_mem_size: "
                << get_device_info<uint64_t>(de, CL_DEVICE_LOCAL_MEM_SIZE)
                << std::endl;
      std::cout << "    global_mem_size: "
                << get_device_info<uint64_t>(de, CL_DEVICE_GLOBAL_MEM_SIZE)
                << std::endl;
      std::cout << "    max_compute_units: "
                << get_device_info<uint64_t>(de, CL_DEVICE_MAX_COMPUTE_UNITS)
                << std::endl;
      std::cout << "    max_work_group_size: "
                << get_device_info<uint64_t>(de, CL_DEVICE_MAX_WORK_GROUP_SIZE)
                << std::endl;
      auto extensions = get_device_info(de, CL_DEVICE_EXTENSIONS);
      size_t pos = 0, new_pos;
      std::cout << "    extensions:" << std::endl;
      while (-1 != (new_pos = extensions.find(' ', pos))) {
        std::cout << "    - " << extensions.substr(pos, new_pos - pos)
                  << std::endl;
        pos = new_pos + 1;
      }
    }
...
struct CLContext {
  ...
  CLContext(int plat, int dev) {
    ...
    cl_int err = -1;
    context =
        clCreateContext(NULL, devices.size(), devices.data(), NULL, NULL, &err);
    if (err < 0) {
...
struct CLBuffer {
  ...
  CLBuffer(CLContext *ctx_, size_t size_, cl_uint type = CL_MEM_READ_WRITE)
      : ctx(ctx_), size(size_) {
    // OpenCL will complain about zero-length buffers:
    if (!size)
      size = 1;
    ...
    if (err < 0) {
      TI_ERROR("Failed to create OpenCL buffer of size {}: {}", size,
               opencl_error(err));
    }
  }
  ...
  void write(const void *ptr, size_t size, size_t offset = 0) {
    cl_int err = clEnqueueWriteBuffer(ctx->cmdqueue, buf, CL_FALSE, offset,
                                      size, ptr, 0, NULL, NULL);
    if (err < 0) {
      TI_ERROR("Failed to write OpenCL buffer offset {} size {}: {}", offset,
               size, opencl_error(err));
    }
  }

  void read(void *ptr, size_t size, size_t offset = 0) {
    cl_int err = clEnqueueReadBuffer(ctx->cmdqueue, buf, CL_TRUE, offset, size,
                                     ptr, 0, NULL, NULL);
    if (err < 0) {
      TI_ERROR("Failed to read OpenCL buffer offset {} size {}: {}", offset,
               size, opencl_error(err));
    }
  }
};
...
struct CLProgram {
  ...
  CLProgram(CLContext *ctx, const std::string &src, std::string options = "")
    ...
    if (err < 0) {
      size_t n;
      clGetProgramBuildInfo(prog, ctx->device, CL_PROGRAM_BUILD_LOG, 0, NULL,
                            &n);
      char log[n + 1];
      clGetProgramBuildInfo(prog, ctx->device, CL_PROGRAM_BUILD_LOG, n,
                            (void *)log, NULL);
      log[n] = 0;
...
struct CLKernel {
  ...
  CLKernel(CLProgram *prog, std::string name)
    ...
    if (err < 0) {
      TI_ERROR("Failed to create OpenCL kernel {}: {}", name,
               opencl_error(err));
    }
  }
  ...
  void set_arg(int index, const void *base, size_t size) {
    ...
    if (err < 0) {
      TI_ERROR("Failed to set kernel argument {} for {}: {}", index, name,
               opencl_error(err));
    }
  }
  ...
  void launch(size_t n_dims,
              const size_t *global_dim,
              const size_t *block_dim) {
    cl_int err = clEnqueueNDRangeKernel(ctx->cmdqueue, kern, n_dims, NULL,
                                        global_dim, block_dim, 0, NULL, NULL);
    if (err < 0) {
      TI_ERROR("Failed to launch OpenCL kernel {} on ({}, {}): {}", name,
               n_dims ? global_dim[0] : 0,
               n_dims && block_dim ? block_dim[0] : 0, opencl_error(err));
    }
  }
...
struct OpenclKernel::Impl {
  ...
  void launch(Context *ctx) {
    ...
    for (int i = 0; i < kernel->args.size(); i++) {
      if (kernel->args[i].is_nparray) {
        auto extr = std::make_unique<CLBuffer>(prog->impl->context.get(),
                                               kernel->args[i].size);
        extr->write(reinterpret_cast<void *>(ctx->args[i]),
                    kernel->args[i].size);
    ...
    for (const auto &[meta, ker] : kernels) {
      ...
      for (int i = 0; i < kernel->args.size(); i++) {
        if (kernel->args[i].is_nparray) {
          ...
          ker->set_arg(i + 2 + kernel->args.size(), ctx->extra_args[i],
                       sizeof(int) * 8);

        } else {
          ker->set_arg(i + 2, &ctx->args[i],
                       data_type_size(kernel->args[i].dt));
        }
      }
    ...
    for (int i = 0; i < kernel->args.size(); i++) {
      if (kernel->args[i].is_nparray) {
        auto const &extr = *extr_iter++;
        extr->read(reinterpret_cast<void *>(ctx->args[i]),
                   kernel->args[i].size);
      }
    }
  }
...
OpenclKernel::OpenclKernel(OpenclProgram *prog,
                           Kernel *kernel,
                           std::vector<OpenclOffloadMeta> const &offloads,
                           std::string const &source)
    : name(kernel->name),
      source(source),
      impl(std::make_unique<Impl>(prog, kernel, offloads, source)) {
}
