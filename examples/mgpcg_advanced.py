...
class MGPCG:
  def __init__(self):
    ...
    self.pixels = ti.var(dt=real, shape=(self.N_gui, self.N_gui))  # image buffer

    self.grid = ti.root.pointer(ti.ijk, [self.N_tot // 4]).dense(ti.ijk, 4).place(self.x, self.p, self.Ap)

    for l in range(self.n_mg_levels):
      self.grid = ti.root.pointer(ti.ijk, [self.N_tot // (4 * 2**l)]).dense(ti.ijk, 4).place(self.r[l], self.z[l])
  ...
  @ti.kernel
  def init(self):
    for i, j, k in ti.ndrange((self.N_ext, self.N_tot - self.N_ext), (self.N_ext, self.N_tot - self.N_ext),
  ...
  def run(self):
    gui = ti.GUI("Multigrid Preconditioned Conjugate Gradients", res=(self.N_gui, self.N_gui))
