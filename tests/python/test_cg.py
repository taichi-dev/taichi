...
def test_cg(ti_dtype):
    ...
    Abuilder = ti.linalg.SparseMatrixBuilder(n,
                                             ...
                                             dtype=ti_dtype)
    np_dtype = np.float32 if ti_dtype == ti.f32 else np.float64
    b = np.ndarray(dtype=np_dtype, shape=n)
    x0 = np.ndarray(dtype=np_dtype, shape=n)
