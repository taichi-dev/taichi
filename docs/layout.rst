...
Arrays (:ref:`scalar_array`) can be *placed* in a specific shape and *layout*.
...
In Taichi, the layout is defined in a recursive manner. See :ref:`snode` for more details about how this works. We suggest starting with the default layout specification (simply by specifying ``shape`` when creating arrays using ``ti.var/Vector/Matrix``),
...
For example, this declares a 0-D array:
...
This declares a 1D array of size ``3``:
...
This declares a 2D array of shape ``(3, 4)``:
...
You may wonder, why not simply specify the ``shape`` of the array? Why bother using the more complex version?
...
Since address spaces are linear in modern computers, for 1D Taichi arrays, the address of the ``i``-th element is simply ``i``.

To store a multi-dimensional array, however, it has to be flattened, in order to fit into the 1D address space.
For example, to store a 2D array of size ``(3, 2)``, there are two ways to do this:
...
Arrays of same size can be placed together.

For example, this places two 1D arrays of size ``3`` (array of structure, AoS):
...
In contrast, this places two array placed separately (structure of array, SoA):
...
However, locality sometimes have a significant impact on the performance, especially when the array is huge.
...
Struct-fors on nested dense data structures will automatically follow their data order in memory. For example, if 2D scalar array ``A`` is stored in row-major order,
...
Struct-for loops on sparse arrays follow the same philosophy, and will be discussed further in :ref:`sparse`.
