...
MetalRuntimeOwned::MetalRuntimeOwned()
    : runtime_module_(
          std::make_unique<taichi::lang::metal::CompiledRuntimeModule>(
              taichi::lang::metal::compile_runtime_module())),
      kernel_manager_(std::make_unique<taichi::lang::metal::KernelManager>(
          make_metal_compute_device_params(*runtime_module_,
                                           host_result_buffer_.data()))),
      device_(kernel_manager_->device().device),
      gfx_runtime_(taichi::lang::gfx::GfxRuntime::Params{
          host_result_buffer_.data(), device_.get()}) {
}
