...
void LevelSet2D::add_sphere(Vector2 center, float radius, bool inside_out) {
	for (auto &ind : get_region()) {
		Vector2 sample = ind.get_pos();
		float dist = (inside_out ? -1 : 1) * (length(center - sample) - radius);
...
void LevelSet2D::add_polygon(std::vector<Vector2> polygon, bool inside_out)
{
	for (auto &ind : get_region()) {
		...
		float dist = ((inside_polygon(p, polygon) ^ inside_out) ? -1 : 1) * (nearest_distance(p, polygon));
		set(ind, std::min(get(ind), dist));
	}
}
...
Vector2 LevelSet2D::get_gradient(const Vector2 &pos) const
{
	...
	const float x_r = x - x_i;
	const float y_r = y - y_i;
	const float gx = lerp(y_r, get(x_i + 1, y_i) - get(x_i, y_i), get(x_i + 1, y_i + 1) - get(x_i, y_i + 1));
	const float gy = lerp(x_r, get(x_i, y_i + 1) - get(x_i, y_i), get(x_i + 1, y_i + 1) - get(x_i + 1, y_i));
...
Array2D<float> LevelSet2D::rasterize(int width, int height) {
	for (auto &p : (*this)) {
		if (std::isnan(p)) {
			printf("Warning: nan in levelset.");
		}
	}
	Array2D<float> out(width, height);
	Vector2 actual_size;
	...
	for (auto &ind : Region2D(0, width, 0, height, Vector2(0.5f, 0.5f))) {
		...
		if (std::isnan(out[ind])) {
			out[ind] = std::numeric_limits<float>::infinity();
		}
	}
	return out;
}
