...
def create_mpm_snow_block(fn):
    ...
    transform = tc_core.Matrix4(1.0).scale_s(2).scale(Vector(2.0, 0.5, 1.0)).translate(Vector(-2, -1, -1))
    vol = VolumeMaterial('voxel', scattering=100, absorption=0, resolution=256, tex=tex,
...
def create_snow_scene(eye_position):
    ...
    width, height = 960 / downsample, 540 / downsample
    camera = Camera('pinhole', width=width, height=height, fov=70,
                    origin=(0, 0, 4), look_at=(0.0, 0.0, 0.0), up=(0, 1, 0))
    ...
    with scene:
        ...
        mesh = Mesh('plane', SurfaceMaterial('emissive', color=(1, 1, 1)),
                    translate=(0.0, 1.0, -1), scale=0.1, rotation=(180, 0, 0))
        scene.add_mesh(mesh)

        fn = '../output/frames/snow-taichi-g10/particles%05d.bin' % 0
