...
public:
  CompiledSpirvSNode run(IRBuilder* builder, 
    ...
    result.root_stype = compute_snode_stype(builder, compiled_structs, 
                                            compiled_structs->snode_descriptors.find(0)->second,
                                            &result.snode_id_struct_stype_tbl,
                                            &result.snode_id_array_stype_tbl);
    return result;
  }
    ...
    if (sn->is_place()) {
      ...
      for (const auto &ch : sn->ch) {
        ...
        if (!ch_sn->is_place()) {
          ...
          Value length = ir_->int_immediate_number(ir_->i32_type(), ch_desc.cells_per_container_pot());
          ir_->declare_global(spv::OpTypeArray, ch_type_array, ch_type, length); // Length
          ir_->decorate(spv::OpDecorate, ch_type_array, 
                        spv::DecorationArrayStride, ch_desc.cell_stride); // Stride
        } else {
          ch_type_array = ch_type;
        }
        ir_->declare_global(spv::OpMemberDecorate, sn_type, cn_cnt++, 
                            spv::DecorationOffset, ch_desc.mem_offset_in_parent_cell); // Offset
        ...
        snode_id_struct_stype_tbl_->insert(std::make_pair(ch_sn->id, std::move(ch_type)));
        TI_ASSERT(snode_id_array_stype_tbl_->find(ch_sn->id) == snode_id_array_stype_tbl_->end());
        snode_id_array_stype_tbl_->insert(std::make_pair(ch_sn->id, std::move(ch_type_array)));
      }

      ir_->declare_global(spv::OpTypeStruct, sn_type, sn_type.snode_child_type_id);
      return sn_type;
    }
  }
};

CompiledSpirvSNode compile_spirv_snode_structs(IRBuilder* builder, 
                                  const CompiledSNodeStructs* compiled_structs) {
