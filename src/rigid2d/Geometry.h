...
class Vector2D {
 public:
  ...
  Vector2D operator-() const { return Vector2D(-x, -y, z); }
  double operator*(const Vector2D &vec) const { return x * vec.x + y * vec.y; }
  double operator%(const Vector2D &vec) const { return x * vec.y - y * vec.x; }
  ...
  double GetLength() const { return sqrt(GetLength2()); }
  ...
  Vector2D GetRotate() const { return Vector2D(-y, x, z); }
  Vector2D GetInverseRotate() const { return Vector2D(y, -x, z); }
  Vector2D GetVector() const { return Vector2D(x, y, 0); }
  Vector2D GetPosition() const { return Vector2D(x, y, 1); }
  Vector2D GetRotated(Angle ang) const {
    return Vector2D(x * ang.c - y * ang.s, x * ang.s + y * ang.c, z);
  }
  void Rotate(Angle ang) { *this = GetRotated(ang); }
...
class Line {
 public:
  ...
  bool IsParallel(const Line &l) const { return abs(l.v % v) < eps; }
...
struct Matrix3x3 {
  ...
  void SetAsAxisTransform(Vector2D movement, double rotationAngle) {
    a[0][0] = cos(rotationAngle);
    a[0][1] = -sin(rotationAngle);
