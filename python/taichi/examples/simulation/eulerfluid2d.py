...
def divergence(vf: ti.template(), divf: ti.template()):
    for i, j in vf:
        divf[i, j] = 0.5 * (vf[i + 1, j][0] - vf[i - 1, j][0] +
                            vf[i, j + 1][1] - vf[i, j - 1][1])


@ti.kernel
def pressure_iteration(divf: ti.template(), pf: ti.template(),
                       new_pf: ti.template()):
    for i, j in pf:
        new_pf[i, j] = (pf[i + 1, j] + pf[i - 1, j] + pf[i, j - 1] +
                        pf[i, j + 1] - divf[i, j]) / 4
...
@ti.kernel
def pressure_projection(pf: ti.template(), vf: ti.template(),
                        vf_new: ti.template()):
    for i, j in vf:
        vf_new[i, j] = vf[i, j] - ti.Vector(
            [(p_with_boundary(pf, i + 1, j, eulerSimParam['shape']) -
              p_with_boundary(pf, i - 1, j, eulerSimParam['shape'])) / 2.0,
             (p_with_boundary(pf, i, j + 1, eulerSimParam['shape']) -
              p_with_boundary(pf, i, j - 1, eulerSimParam['shape'])) / 2.0])
...
def p_with_boundary(pf: ti.template(), i: int, j: int, shape) -> ti.f32:
    if (i == j == 0) or (i == shape[0] - 1 and j == shape[1] - 1) or (
            i == 0 and j == shape[1] - 1) or (i == shape[0] - 1 and j == 0):
        pf[i, j] = 0.0
    elif i == 0:
        pf[0, j] = pf[1, j]
    elif j == 0:
...
def mouse_interaction(prev_posx: int, prev_posy: int):
    ...
    mouse_addspeed(mousePos_x, mousePos_y, prev_posx, prev_posy, mouseRadius,
                   velocities_pair.cur, velocities_pair.nxt)
...
@ti.kernel
def mouse_addspeed(cur_posx: int, cur_posy: int, prev_posx: int,
                   prev_posy: int, mouseRadius: float, vf: ti.template(),
                   new_vf: ti.template()):
    for i, j in vf:
...
def pressure_step():
    ...
    pressure_projection(pressure_pair.cur, velocities_pair.cur,
                        velocities_pair.nxt)
    velocities_pair.swap()
    apply_vel_bc(velocities_pair.cur)
...
init_field()
apply_vel_bc(velocities_pair.cur)
window = ti.GUI("Euler 2D Simulation",
                res=(eulerSimParam['shape'][0], eulerSimParam['shape'][1]))
...
while window.running:
    ...
    mouse_prevposx, mouse_prevposy = mouse_interaction(mouse_prevposx,
                                                       mouse_prevposy)
