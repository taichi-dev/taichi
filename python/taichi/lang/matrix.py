...
class Matrix(TaichiOperations):
            ...
            if len(n) == 0:
                ...
                if impl.inside_kernel():
                    ...
                    if keep_raw:
                        ...
                        if ti.current_cfg().dynamic_index and not ti.is_extension_supported(ti.cfg.arch,
                                                                                            ti.extension.dynamic_index):
                            ti.current_cfg().dynamic_index = False
                            warning('Backend ' + str(ti.cfg.arch) + ' doesn\'t support dynamic index, disable it automatically.')
                        if in_python_scope(
    ...
    def inverse(self):
        ...
        if self.n == 1:
            ...
            return inv_det * Matrix([[self(1, 1), -self(0, 1)],
                                     [-self(1, 0), self(0, 0)]], disable_local_tensor=True).variable()
        elif self.n == 3:
            n = 3
    ...
    def transpose(self):
        ...
        ret = Matrix([[self[i, j] for i in range(self.n)]
                      for j in range(self.m)], disable_local_tensor=True)
        return ret
    ...
    def zero(dt, n, m=None):
        ...
        """
        if m is None:
            return Vector([ti.cast(0, dt) for _ in range(n)], disable_local_tensor=True)
        else:
            return Matrix([[ti.cast(0, dt) for _ in range(m)]
                           for _ in range(n)], disable_local_tensor=True)
    ...
    def one(dt, n, m=None):
        ...
        """
        if m is None:
            return Vector([ti.cast(1, dt) for _ in range(n)], disable_local_tensor=True)
        else:
    ...
    def unit(n, i, dt=None):
        ...
        if dt is None:
            dt = int
        assert 0 <= i < n
        return Matrix([ti.cast(int(j == i), dt) for j in range(n)], disable_local_tensor=True)
    ...
    def identity(dt, n):
        ...
        return Matrix([[ti.cast(int(i == j), dt) for j in range(n)]
                       for i in range(n)], disable_local_tensor=True)
    ...
    def outer_product(self, other):
        ...
        ret = Matrix([[self[i] * other[j] for j in range(other.n)]
                      for i in range(self.n)], disable_local_tensor=True)
        return ret
