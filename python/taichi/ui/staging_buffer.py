...
@ti.kernel
def copy_image_f32_to_u8(src:ti.template(),dst:ti.template(),num_components:ti.template()):
    for i,j in src:
...
@ti.kernel
def copy_image_u8_to_u8(src:ti.template(),dst:ti.template(),num_components:ti.template()):
    for i,j in src:
        for k in ti.static(range(num_components)):      
            dst[i,j][k] = src[i,j][k]
...
def to_u8_rgba(image):
    if not hasattr(image, 'n') or image.m != 1:
        raise Exception(f'the input image needs to be a Vector field (matrix with 1 column)')
    if len(image.shape) != 2:
        raise Exception(f"the shape of the image must be of the form (width,height)")
