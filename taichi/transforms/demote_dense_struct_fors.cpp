...
void convert_to_range_for(OffloadedStmt *offloaded) {
  ...
  std::copy(std::begin(start0), std::end(start0), std::begin(start));
  ...
  for (int i = 0; i < (int)snodes.size(); i++) {
    ...
    for (int j = 0; j < (int)physical_indices.size(); j++) {
      ...
      start[p] -= ext.num_bits;
      auto multiplier =
          body_header.push_back<ConstStmt>(TypedConstant(1 << start[p]));
  ...
  for (int i = 0; i < (int)snodes.size(); i++) {
    ...
    for (int j = 0; j < (int)physical_indices.size(); j++) {
      auto p = physical_indices[j];
      start[p] -= snode->extractors[p].num_bits;
      auto num_elements = snode->extractors[p].num_elements << start[p];
