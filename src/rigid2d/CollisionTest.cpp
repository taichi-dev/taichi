...
void Physics::TestCollision(Polygon *a, Polygon *b) {
  ...
  for (int i = 0; i < a->nPoints; i++) {
    ...
    if (max0 < min1 || max1 < min0)
      return;
    double d = max0 - min1;
    if (min1 < max0 && (sgn(d - minDepth) < 0) ||
        sgn(d - minDepth) == 0 && a->mass > b->mass) {
      flg = 0;
      minDepth = d;
      n = normal;
      edgeId = i;
  ...
  for (int i = 0; i < b->nPoints; i++) {
    ...
    double d = max0 - min1;
    if ((min1 < max0 && (sgn(d - minDepth) < 0)) ||
        (sgn(d - minDepth) == 0 && b->mass > a->mass)) {
      flg = 1;
      minDepth = d;
      n = normal;
      edgeId = i;
  ...
  flg = -1;
  for (int i = 0; i < b->nPoints; i++) {
    if (n * (b->GetPoint(i) - edge.a) < 0 &&
        n * (b->GetPoint((i + 1) % b->nPoints) - edge.a) < 0) {
      flg = i;
      break;
    }
  }
  if (flg != -1) {
    ...
    Vector2D A = a->GetPoint(i), B = a->GetPoint((i + 1) % a->nPoints),
             C = b->GetPoint(j), D = b->GetPoint((j + 1) % b->nPoints);
    ...
    if (sgn(a->GetNormal(i) * b->GetNormal(j) + 1.0) != 0) {
      ...
      constraints.push_back(new Contact(
          a, b, (cons[1].second + cons[2].second - Vector2D::Origin) * 0.5, n,
          minDepth));
    }
  } else {
    Vector2D retP;
...
void Physics::TestCollision(Circle *a, Circle *b) {
  ...
  constraints.push_back(
      new Contact(a, b, a->GetCentroidPosition() + n * a->radius, n, depth));
}
...
void Physics::TestCollision(Circle *a, Polygon *b) {
  ...
  for (int i = 0; i < b->nPoints; i++) {
    ...
    Vector2D tempP = tempEdge.a +
                     (a->GetCentroidPosition() - tempEdge.a) *
                         tempEdge.v.GetDirection() * tempEdge.v.GetDirection();
    if (tempEdge.IsInside(tempP) && max0 < max1 && min1 < max0 &&
        max0 - min1 < minDepth) {
      minDepth = max0 - min1;
      n = normal;
  ...
  for (int i = 0; i < b->nPoints; i++) {
    ...
    if (a->IsPointInside(p)) {
      ...
      constraints.push_back(new Contact(
          a, b, p, (p - a->GetCentroidPosition()).GetDirection(), depth));
      return;
    }
  }
}
