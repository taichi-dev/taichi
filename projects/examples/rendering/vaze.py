...
def create_scene():
  ...
  with scene:
    ...
    mesh = tc.geometry.create_mesh_from_functions(
        (150, 150), surf, smooth=False)
    scene.add_mesh(
        tc.Mesh(
            mesh,
    ...
    mesh = tc.Mesh(
        'plane',
        tc.SurfaceMaterial('emissive', color=(0.01, 0.02, 0.04)),
        translate=(10, 10, 30),
        scale=3,
        rotation=(-110, 45, 0))
    scene.add_mesh(mesh)
    ...
    with tc.transform_scope(
        translate=(9, 10, -10), scale=(0.3, 1.3, 0.3), rotation=(88, -27, 0)):
      mesh = tc.Mesh('plane',
