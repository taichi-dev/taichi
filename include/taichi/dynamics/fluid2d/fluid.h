...
public:
    struct Particle {
        ...
        template <int k> static real get_velocity(const Particle &p, const Vector2 &delta_pos) { return p.velocity[k]; }
        template <int k> static real get_affine_velocity(const Particle &p, const Vector2 &delta_pos) { return p.velocity[k] + dot(p.c[k], delta_pos); }
        static real get_signed_distance(const Particle &p, const Vector2 &delta_pos) { return length(delta_pos) - p.radius; }
