...
vt0_aos  = ti.Vector.ndarray(n=3,    dtype=ti.f32, shape=N, layout=ti.Layout.AOS) #(10,3)
# (not works) memory_layout (SOA): [x,x,..,x],[y,y,..,y],[z,z,..,z]
vt0_soa  = ti.Vector.ndarray(n=3,    dtype=ti.f32, shape=N, layout=ti.Layout.SOA) #(3,10)
# memory_layout (AOS/SOA): [x,y,z],[x,y,z]....[x,y,z]
vt1_aos = ti.Vector.ndarray(n=1,    dtype=ti.f32, shape=3 * N, layout=ti.Layout.AOS) #(30,1)
vt1_soa = ti.Vector.ndarray(n=1,    dtype=ti.f32, shape=3 * N, layout=ti.Layout.SOA) #(1,30)
vt2_aos = ti.Vector.ndarray(n=3 * N,dtype=ti.f32, shape=(),layout=ti.Layout.AOS) #(30,)
vt2_soa = ti.Vector.ndarray(n=3 * N,dtype=ti.f32, shape=(),layout=ti.Layout.SOA) #(30,)
...
# memory_layout (AOS/SOA): [x,y,z],[x,y,z]....[x,y,z]
mt0_aos = ti.Matrix.ndarray(n=10,m=3,dtype=ti.f32,shape=(),  layout=ti.Layout.AOS) #(10,3)
mt0_soa = ti.Matrix.ndarray(n=10,m=3,dtype=ti.f32,shape=(),  layout=ti.Layout.SOA) #(10,3)
mt1_aos = ti.Matrix.ndarray(n=1,m=3,dtype=ti.f32,shape=(10,),layout=ti.Layout.AOS) #(10,1,3)
mt3_aos = ti.Matrix.ndarray(n=3,m=1,dtype=ti.f32,shape=(10,),layout=ti.Layout.AOS) #(10,3,1)

# (not works) memory_layout (AOS/SOA): [x,x,..,x],[y,y,..,y],[z,z,..,z]
mt1_soa = ti.Matrix.ndarray(n=1,m=3,dtype=ti.f32,shape=(10,),layout=ti.Layout.SOA) #(1,3,10)
mt3_soa = ti.Matrix.ndarray(n=3,m=1,dtype=ti.f32,shape=(10,),layout=ti.Layout.SOA) #(3,1,10)
mt2_aos = ti.Matrix.ndarray(n=3,m=10,dtype=ti.f32,shape=(), layout=ti.Layout.AOS) #(3,10)
mt2_soa = ti.Matrix.ndarray(n=3,m=10,dtype=ti.f32,shape=(), layout=ti.Layout.SOA) #(3,10)
