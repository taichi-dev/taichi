 ...
 using BasicStmtVisitor::visit;
  ...
  void visit(InternalFuncStmt *stmt) override {
    if (stmt->func_name == "l2g") {
      VecStatement block;
      SNode *l2g = stmt->args[0]->cast<GlobalLoadStmt>()->src->cast<GlobalPtrStmt>()->snodes[0];
      SNode *offset = stmt->args[2]->cast<GlobalLoadStmt>()->src->cast<GlobalPtrStmt>()->snodes[0];
      ...
      Stmt* index = block.push_back<BinaryOpStmt>(BinaryOpType::add, total_v, stmt->args[1]);
