...
class MeshElementField:
    ...
    @python_scope
    def __setitem__(self, indices, element): # TODO(changyu): handle reordering case
...
class MeshElement:
    ...
    def place(self, 
        ...
        for key, dtype in members.items():
            self.attr_dict[key] = MeshAttrType(key, dtype, reordering, needs_grad)
...
class MeshRelationAccess:
    ...
    @property
    def size(self):
        return impl.Expr(_ti_core.get_relation_size(self.mesh.mesh_ptr, 
                                                  self.from_index.ptr, 
