...
Q = ti.Vector(4, dt=real, shape=(
    N,
    N))  # cell average conserved variables vector [rho, rho*u, rho*v, rho*e]
Q_old = ti.Vector(4, dt=real, shape=(N, N))
W = ti.Vector(
    4, dt=real,
    shape=(N, N))  # cell average primitive variables vector [rho, u, v, p]
...
def paint():
    for i, j in img:
        ...
        if img_field == 0:  # density
            ...
            img[i, j] = ti.log(
                ti.sqrt(((Q[ii + 1, jj][0] - Q[ii - 1, jj][0]) / h)**2 +
