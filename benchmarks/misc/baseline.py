...
class TestResult:
    ...
    def time2mdtableline(self):
        ...
        string += ''.join(str(round(time,4))+'|' for time in self.min_time_in_us)
        return string
...
class CaseImpl:
    ...
    def run(self):
        for arch in self.archs:
            ...
            for dtype in self.data_type:
                print("TestCase[%s.%s.%s]" %(self.func.__name__, ti.core.arch_name(arch), dtype2str[dtype]))
                result = TestResult(self.name,arch,dtype,self.data_size)
                for size in self.data_size:
                    print( "data_size = %s" % (size2str(size)))
                    result.min_time_in_us.append(self.func(arch, dtype, size, test_repeat))
                    time.sleep(0.2)
        ...
        for arch in self.archs:
            for dtype in self.data_type:
                for idx in range(len(self.data_size)):
                        print("    test_case:[%s] arch:[%s] dtype:[%s] dsize:[%7s] >>> time:[%4.4f]" 
                        %(self.name, 
                        ti.core.arch_name(arch), 
                        dtype2str[dtype], 
    ...
    def save2markdown(self,arch):
        ...
        gap    = '||(time:us)' + ''.join('|' for i in range(len(self.data_size)-1))
        lines.append(gap)
        return lines
...
class Baseline:
    ...
    def __init__(self):
        for case in test_cases:
            self.case_impl.append(CaseImpl(case,test_archs,test_dtype,test_dsize))
    def md_table_header(suite_name,arch,test_dsize):
        col = len(test_dsize)+1
        header = '|' + suite_name + '.' + ti.core.arch_name(arch) + ''.join('|' for i in range(col))
        layout = '|:--:|' +''.join(':--:|' for size in test_dsize)
        size   = '|**data_size**|' +''.join(size2str(size)+'|' for size in test_dsize)
        repeat = '|**repeat**|' +''.join(str(scale_repeat(arch,size,test_repeat))+'|' for size in test_dsize)
    ...
    def mdlines(self):
        ...
        for arch in test_archs:
            lines += self.__class__.md_table_header(self.__class__.__name__,arch,test_dsize)
            for case in self.case_impl:
