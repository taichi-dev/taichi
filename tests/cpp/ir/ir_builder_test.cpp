...
TEST(IRBuilder, ExternalPtr) {
  ...
  lang::launch_kernel(test_prog.prog(), *ker, launch_ctx.get_context());
  EXPECT_EQ(array[0], 2);
  EXPECT_EQ(array[1], 1);
...
TEST(IRBuilder, Ndarray) {
  ...
  lang::launch_kernel(test_prog.prog(), *ker1, launch_ctx1.get_context());
  ...
  lang::launch_kernel(test_prog.prog(), *ker2, launch_ctx2.get_context());
  EXPECT_EQ(array.read_int({0}), 2);
