...
To declare a scalar field, simply call `ti.field(dtype, shape)` where `dtype` is a primitive data type as explained in [type system](../type_system/type.md), and `shape` is a tuple of integers.
