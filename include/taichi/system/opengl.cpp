...
GLuint load_program(string vertex_fn, string fragment_fn) {
	GLint v_shader = load_shader(GL_VERTEX_SHADER, "shaders/" + vertex_fn +
		".vertex"), f_shader = load_shader(
			GL_FRAGMENT_SHADER, "shaders/" + fragment_fn + ".fragment");
...
GLWindow::GLWindow(Config config) {
	...
	window = glfwCreateWindow(width, height, title.c_str(), nullptr, first_window);
	glfwSetWindowPos(window, x_start_position + config.get("offset_x", 300), config.get("offset_y", 300));
