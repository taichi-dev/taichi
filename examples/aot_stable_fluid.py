...
_dye_buffer = ti.Vector.field(4, float, shape=(res, res))
_new_dye_buffer = ti.Vector.field(4, float, shape=(res, res))
...
def apply_impulse(vf: ti.template(), dyef: ti.template(),
                  mpx: ti.f32, mpy: ti.f32, mdx: ti.f32, mdy: ti.f32, colorr: ti.f32, colorg: ti.f32, colorb: ti.f32):
    g_dir = -ti.Vector([0, 9.8]) * 300
    for i, j in vf:
        omx, omy = mpx, mpy
        mdir = ti.Vector([mdx, mdy])
        ...
        momentum = (mdir * f_strength * factor + 0 * g_dir * a / (1 + a)) * dt
...
m.add_kernel(apply_impulse, name='apply_impulse=v0d0', template_args={'vf': velocities_pair.cur, 'dyef': dyes_pair.cur})
m.add_kernel(apply_impulse, name='apply_impulse=v1d1', template_args={'vf': velocities_pair.nxt, 'dyef': dyes_pair.nxt})
