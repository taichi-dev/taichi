...
TEST(Simplify, SimplifyLinearizedWithTrivialInputs) {
  ...
  irpass::type_check(block.get(), kernel->program->global_compile_config());
  EXPECT_EQ(block->size(), 7);

  irpass::simplify(
      block.get(),
      kernel->program->global_compile_config());  // should lower linearized
  // EXPECT_EQ(block->size(), 11);  // not required to check size here

  irpass::constant_fold(block.get(), kernel->program->global_compile_config(),
                        {kernel->program});
  irpass::alg_simp(block.get(), kernel->program->global_compile_config());
  irpass::die(block.get());  // should eliminate consts
  irpass::simplify(block.get(), kernel->program->global_compile_config());
  irpass::whole_kernel_cse(block.get());
  if (kernel->program->global_compile_config().advanced_optimization) {
