        ...
        for (int j = 0; j < 3; j++) {
          for (int k = 0; k < 3; k++) {
            ...
            grid[base_coord.x + i][base_coord.y + j][base_coord.z + k] +=
                w[i].x * w[j].y * (mv + Vector4(affine * dpos, 0));
          }
        }
      }
    }
...
#define broadcast(s, i) _mm_shuffle_ps((s), (s), 0x55 * (i))
  void p2g_intrinsics() {
    ...
    for (int p_i = 0; p_i < n_particles; p_i++) {
      ...
      const Matrix3f apic_b_inv_d_mass = p.C * (4.0_f * mass);
      const __m128 mass_v = _mm_mul_ps(_mm_set1_ps(mass), v);

      auto stress = Matrix3(p.J - 1);
...
#define LOOP(node_id)                                                        \
  {                                                                          \
    ...
    auto &g =                                                                \
        grid[0][0][node_id / 9 * n * n + node_id / 3 % 3 * n + node_id % 3]; \
