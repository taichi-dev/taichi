...
void AMCMCPPMRenderer::render_stage() {
  ...
  if (!mc_initialized) {
    ...
    while (true) {
      ...
      if (emitted % 100000 == 0) {
        ...
        std::cout << emitted << " photons emitted without any visible one."
                  << std::endl;
      }
  ...
  for (int64 i = 0; i < num_photons_per_stage; i++) {
    ...
    //   3. Replica exchange the two chains. The exchange prob. is 1 if uniform
    //   state visible, and 0 otherwise.
    // ----------------------------------------

    // So we can do steps 2 & 3 first. If the uniform state is visible, we
    // always do replica exchange, thus
    // there's no need for step 1. Also, note that the uniform chain is always
    // mutated in an uncorrelated
    // manner, i.e. uniform random choice from the unit hypercube, we do not
    // need to store the uniform
    // chain state.
    ...
    // the normalization factor for the visibility chain is
    // normalizer.get_average().
