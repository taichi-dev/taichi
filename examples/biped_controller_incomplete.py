...
controller_outputs1 = scalar()
controller_outputs2 = scalar()
...
def place():
  ...
  ti.root.dense(ti.k, n_particles).place(A)
  ...
  ti.root.dense(ti.i, num_groups).place(controller_bias, controller_outputs1, controller_outputs2)
  ...
  for i in range(num_groups):
    for j in range(num_groups):
      for k in ti.static(range(dim)):
        controller_outputs1[i].atomic_add(controller_weights[i, j * 2 * dim + k] * x_avg(k)[j])
      for k in ti.static(range(dim)):
        controller_outputs1[i].atomic_add(controller_weights[i, j * 2 * dim + dim + k] * v_avg(k)[j])

@ti.kernel
def controller_b():
  for i in range(num_groups):
    controller_outputs2[i] = controller_outputs1[i] + controller_bias[i]
