...
class LevelSet2D:
  ...
  def add_sphere(self, center, radius, inside_out=False):
    ...
    self.levelset.add_sphere(
        Vector(center.x / self.delta_x, center.y / self.delta_x),
        radius / self.delta_x, inside_out)

  def add_polygon(self, polygon, inside_out):
    self.levelset.add_polygon(
        make_polygon(polygon, 1.0 / self.delta_x), inside_out)
  ...
  def get_image(self, width, height, color_255):
    if self.cache_image is None:
      self.cache_image = array2d_to_image(self.levelset, width, height,
                                          color_255)
