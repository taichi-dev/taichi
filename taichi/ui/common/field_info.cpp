...
DevicePtr get_device_ptr(taichi::lang::Program *program, SNode *snode) {
  ...
  Thus, the only situation where this assumption may be violated is for
  set_image(), because the image isn't part of the VBO. Using this assumption,
  we will compute the offset of this field relative to the begin of the root
  buffer.
  */
