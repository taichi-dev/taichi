...
class TaichiWindow(pyglet.window.Window):
  ...
  def __init__(self, width, height):
    super(TaichiWindow, self).__init__(
        width=width,
        height=height,
        fullscreen=False,
        caption='Taichi',
        config=pyglet.gl.Config(
            sample_buffers=0, samples=0, double_buffer=True))
    self.levelset_resolution = 100
    self.scale = 1.0 / self.levelset_resolution
    self.levelset = LevelSet2D(self.levelset_resolution,
                               self.levelset_resolution, self.scale)
  ...
  def on_draw(self):
    ...
    pyglet.graphics.draw(self.width * self.height, gl.GL_POINTS,
                         ('v2i', positions), ('c3f', colors))
