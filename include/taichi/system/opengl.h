...
#define CGL                       \
  {                               \
    ...
    if (code != GL_NO_ERROR) {    \
      printf("GL Error: ");       \
      gl_print_error_code(code);  \
      error("GL error detected"); \
    }                             \
  };
...
class GLWindow {
 public:
  ...
  static void general_mouse_move_callback(GLFWwindow *window,
                                          double x,
                                          double y) {
    window_to_instance[window]->mouse_move_callback((float)x, (float)y);
  }

  static void general_keyboard_callback(GLFWwindow *window,
                                        int key,
                                        int scancode,
                                        int action,
                                        int mods) {
