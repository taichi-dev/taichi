...
class FluidSimulator(Simulator):
  ...
  def __init__(self, **kwargs):
    ...
    self.simulation_width, self.simulation_height = self.resolution[
        0], self.resolution[1]
    ...
    while x < x_1:
      ...
      while y < y_1:
        ...
        if vel_eval:
          ...
          vel = (0, 0)
        samples.append(
            tc.core.FluidParticle(
                Vector(x / self.delta_x, y / self.delta_x),
                Vector(vel[0], vel[1])))
      x += delta_x
  ...
  def add_particles_sphere(self, center, radius, vel_eval=None):
    positions = tc.core.points_inside_sphere(
        tc.core.make_range(.25 * self.delta_x,
                           self.resolution[0] * self.delta_x,
                           self.delta_x / self.sample_rate),
        tc.core.make_range(.25 * self.delta_x,
                           self.resolution[1] * self.delta_x,
    ...
    for p in positions:
      ...
      samples.append(
          tc.core.FluidParticle(
              Vector(p.x / self.delta_x, p.y / self.delta_x),
              Vector(vel[0], vel[1])))
  ...
  def get_levelset_images(self, width, height, color_scheme):
    images = []
    images.append(
        self.levelset.get_image(width, height, color_scheme['boundary']))
    liquid_levelset = self.get_liquid_levelset()
    images.append(
        array2d_to_image(liquid_levelset, width, height, color_scheme[
            'liquid']))
    if self.show_pressure:
      images.append(
          array2d_to_image(self.get_pressure(), width, height, (255, 0, 0, 128),
                           (0, 1)))
    return images, []
...
class SmokeSimulator(FluidSimulator):
  ...
  def get_levelset_images(self, width, height, color_scheme):
    ...
    images.append(
        self.levelset.get_image(width, height, color_scheme['boundary']))
    images.append(
        array2d_to_image(self.get_density(), width, height, color_scheme[
            'smoke'], (0, 5)))
    return images
