...
def cast(obj, dtype):
    ...
    Args:
        obj (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
...
def bit_cast(obj, dtype):
    ...
    Args:
        ...
        dtype (Union[`~taichi.types.primitive_types`]): Data type defined in \
            `~tachi.types.primitive_types`, must have the same precision bits \
                as the input type (hence `f32` -> `f64` is not allowed).
...
def round(a):  # pylint: disable=redefined-builtin
    ...
    Args:
        a (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
            Input scalar or matrix.
...
def random(dtype=float):
    ...
    For float types return a random float in the half-open interval [0, 1).

    For int types return a random integer in the half-open interval [0, 2^32) for 32-bit,
    or a random integer in the half-open interval [0, 2^64) for 64-bit.
...
def mod(x1, x2):
    ...
    Args:
        x1 (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
            Dividend scalar or matrix.

        x2 (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
            Divisor scalar or matrix. If both `x1` and `x2` are matrices they must have the same shape.
...
def floordiv(x1, x2):
    ...
    Args:
        x1 (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
            The numerator.
        x2 (Union[:class:`~taichi.lang.expr.Expr`, :class:`~taichi.lang.matrix.Matrix`]): \
            The denominator. If both `x1` and `x2` are matrices they must have the same shape.
