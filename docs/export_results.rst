...
- ``ti.imwrite`` can export Taichi arrays (``ti.Matrix``, ``ti.Vector``, ``ti.var``) and numpy arrays ``np.ndarray``.
...
- Meanwhile, the resulted image type (grayscale, RGB, or RGBA) is determined by **the number of channels in the input array**, i.e., the length of the third dimension (``array.shape[2]``).
- In other words, a array that has shape ``(w, h)`` or ``(w, h, 1)`` will be exported as a grayscale image.
- If you want to export ``RGB`` or ``RGBA`` images instead, the input array should have a shape ``(w, h, 3)`` or ``(w, h, 4)`` respectively.

.. note::

    All Taichi arrays have their own data types, such as ``ti.u8`` and ``ti.f32``. Different data types can lead to different behaviors of ``ti.imwrite``. Please check out :ref:`gui` for more details.
