...
.. code-block:: python
  ...
  def my_kernel():
    for i, j in array_a:
      array_b[i, j] = some_function(array_a[i, j])

The arrays and function be aliased to new names with ``ti.static``:

.. code-block:: python

  @ti.kernel
  def my_kernel():
    a, b, fun = ti.static(array_a, array_b, some_function)
    for i,j in a:
      b[i,j] = fun(a[i,j])
...
For example, consider class kernel to compute the 2-D laplacian of some array:
...
.. note::
  ...
  Here, we are using it for *compile-time const values*, i.e. the **array/function handles** are constants at compile time.
