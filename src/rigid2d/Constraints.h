...
class Spring : public Force {
 public:
  ...
  Spring(Object *objectA,
    ...
    if (L < 0)
      this->L = (objectA->transformToWorld(r0) - objectB->transformToWorld(r1))
                    .GetLength();
    damping = 150;
  }
  void Apply(double T) {
    ...
    double v0 = n * v;
    Vector2D impulse =
        (-damping * v0 * T + T * strength * (L - l)) * n.GetDirection();
...
#define TYPE_CONTACT 1
  ...
  double depth;
  Contact(Shape *a, Shape *b, Vector2D p, Vector2D n, double depth)
      : shapeA(a),
        objectA(a->object),
        shapeB(b),
        objectB(b->object),
  ...
  void PreprocessVelocity() {
    ...
    double J =
        ...
        (objectA->invMass + objectB->invMass +
         sqr(r0 % n) * objectA->invInertia + sqr(r1 % n) * objectB->invInertia);
    if (J < 0)
      return;
    ...
    if (settings.frictionSwitch) {
      ...
      double j = -(v10 * tao) / (objectA->invMass + objectB->invMass +
                                 sqr(r0 % tao) * objectA->invInertia +
                                 sqr(r1 % tao) * objectB->invInertia);
  ...
  void ProcessPosition() {
    Vector2D r0 = p - objectA->position, r1 = p - objectB->position;
    double correctiveJ =
        (0.5 * max(0.0, depth - 0.1) / timeInterval) /
        (objectA->invMass + objectB->invMass +
         sqr(r0 % n) * objectA->invInertia + sqr(r1 % n) * objectB->invInertia);
    if (correctiveJ > 0) {
  ...
  Constraint *Copy() { return new Contact(*this); }
  bool Linking(Object *object) {
    if (object == objectA || object == objectB)
      return true;
    return false;
  }
...
class DistanceConstraint : public Constraint {
 ...
 public:
  ...
  DistanceConstraint(Object *objectA, Vector2D r0, Object *objectB, Vector2D r1)
      : L((objectA->transformToWorld(r0.GetPosition()) -
           objectB->transformToWorld(r1.GetPosition()))
              .GetLength()),
        objectA(objectA),
        r0(r0),
        objectB(objectB),
  ...
  void ProcessVelocity() {
    ...
    double J;
    double v =
        n * (objectB->GetPointVelocity(q) - objectA->GetPointVelocity(p));
    J = v / (objectA->invMass +
             sqr(objectA->transformToWorld(r0) % n) * objectA->invInertia +
             objectB->invMass +
             sqr(objectB->transformToWorld(r1) % n) * objectB->invInertia);
  ...
  Constraint *Copy() { return new DistanceConstraint(*this); }
  void Redraw() {
    Vector2D p = objectA->position + objectA->transformToWorld(r0);
    Vector2D q = objectB->position + objectB->transformToWorld(r1);
