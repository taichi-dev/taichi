...
llvm::Type *TaichiLLVMContext::get_data_type(DataType dt) {
  ...
  if (dt->is_primitive(PrimitiveTypeID::i8) ||
    ...
    //               "Real matrix or dynamic index not enabled but got TensorType");
    ...
    // Return type is <element_type * num_elements> if use real matrix,
    // otherwise [element_type * num_elements].
    if (config_->real_matrix) {
      return llvm::VectorType::get(element_type, num_elements, /*scalable=*/false);
    }
