...
namespace spirv {
  ...
  std::vector<uint32_t> IRBuilder::finalize() {
    ...
    if (float_atomic_add_.id > 0) {
      data.insert(data.end(), float_atomic_add_function_.begin(), float_atomic_add_function_.end());
    }
    return data;
  }
  ...
  SType IRBuilder::get_struct_array_type(const SType& value_type, uint32_t num_elems) {
    ...
    if (num_elems != 0) {
      Value length = uint_immediate_number(
                        get_primitive_type(get_data_type<uint32>()), 
                        num_elems);
  ...
  Value IRBuilder::buffer_argument(const SType& value_type, uint32_t descriptor_set,
                                   uint32_t binding) {
    // NOTE: BufferBlock was deprecated in SPIRV 1.3
    // use StorageClassStorageBuffer instead.
#if SPV_VERSION >= 0x10300
  ...
  Value IRBuilder::get_work_group_size(uint32_t dim_index) {
    ...
    Value ptr = this->make_value(spv::OpAccessChain, pint_type, gl_work_group_size,
  ...
  void IRBuilder::set_work_group_size(const std::array<int, 3> group_size) {
    ...
    ib_.begin(spv::OpConstantComposite).add_seq(t_v3_uint_, gl_work_group_size, size_x, size_y, size_z).commit(&global_);
    this->decorate(spv::OpDecorate, gl_work_group_size, spv::DecorationBuiltIn, spv::BuiltInWorkgroupSize);
  }
  ...
  Value IRBuilder::get_num_work_groups(uint32_t dim_index) {
    if (gl_num_work_groups.id == 0) {
      ...
      ib_.begin(spv::OpVariable)
        ...
        .commit(&global_);
      this->decorate(spv::OpDecorate, gl_num_work_groups, spv::DecorationBuiltIn, spv::BuiltInNumWorkgroups);
    }
    SType pint_type = this->get_pointer_type(t_uint32_, spv::StorageClassInput);
    Value ptr = this->make_value(spv::OpAccessChain, pint_type, gl_num_work_groups,
                              uint_immediate_number(t_uint32_, static_cast<uint64_t>(dim_index)));
  ...
  Value IRBuilder::get_global_invocation_id(uint32_t dim_index) {
    if (gl_global_invocation_id.id == 0) {
      ...
      ib_.begin(spv::OpVariable)
        ...
        .commit(&global_);
      this->decorate(spv::OpDecorate, gl_global_invocation_id, spv::DecorationBuiltIn, spv::BuiltInGlobalInvocationId);
    }
    SType pint_type = this->get_pointer_type(t_uint32_, spv::StorageClassInput);
    Value ptr = this->make_value(spv::OpAccessChain, pint_type, gl_global_invocation_id,
                              uint_immediate_number(t_uint32_, static_cast<uint64_t>(dim_index)));
...
#define DEFINE_BUILDER_CMP_UOP(_OpName, _Op)                                                    \
  ...
  Value IRBuilder::cast(const SType& dst_type, Value value) {
    ...
    if (from->is_primitive(PrimitiveTypeID::u1)) { // Bool
      if(is_integral(to) && is_signed(to)) { // Bool -> Int
        ...
        TI_ERROR("do not support type cast from {} to {}", from.to_string(), to.to_string());
        return Value();
      }
    ...
    } else if (is_integral(from) && is_signed(from) && is_integral(to) && is_signed(to)) { // Int -> Int
      return make_value(spv::OpSConvert, dst_type, value); 
    } else if (is_integral(from) && is_unsigned(from) && is_integral(to) && is_unsigned(to)) { // UInt -> UInt
      return make_value(spv::OpUConvert, dst_type, value);
    } else if (is_integral(from) && is_unsigned(from) && is_integral(to) && is_signed(to)) { // UInt -> Int
      if (data_type_bits(from) != data_type_bits(to)) {
        ...
        value = make_value(spv::OpUConvert, get_primitive_type(to_signed(from)), value);
      }
      return make_value(spv::OpBitcast, dst_type, value);
    } else if (is_integral(from) && is_signed(from) && is_integral(to) && is_unsigned(to)) { // Int -> UInt
      if (data_type_bits(from) != data_type_bits(to)) {
        value = make_value(spv::OpSConvert, get_primitive_type(to_unsigned(from)), value);
      }
      return make_value(spv::OpBitcast, dst_type, value);
  ...
  Value IRBuilder::alloca_variable(const SType& type) {
    ...
    Value ret = new_value(ptr_type, ValueKind::kVariablePtr);
    ib_.begin(spv::OpVariable).add_seq(type, ret, spv::StorageClassFunction).commit(&func_header_);
    return ret;
  }

  Value IRBuilder::load_variable(Value pointer, const SType& res_type) {
    TI_ASSERT(pointer.flag == ValueKind::kVariablePtr || pointer.flag == ValueKind::kStructArrayPtr);
  ...
  Value IRBuilder::float_atomic_add() {
    if (float_atomic_add_.id == 0) {
      ...
      SType addr_ptr_type = get_pointer_type(t_int32_, spv::StorageClassStorageBuffer);
      ...
      declare_global(spv::OpTypeFunction, float_atomic_add_func_type, t_fp32_, addr_ptr_type, data_type);
      
      // function begin
      auto &func_ = float_atomic_add_function_;

      // function header
      ib_.begin(spv::OpFunction).add_seq(t_fp32_, float_atomic_add_, 0, float_atomic_add_func_type).commit(&func_);
      ...
      ib_.begin(spv::OpFunctionParameter).add_seq(addr_ptr_type, addr_ptr).commit(&func_);
      ib_.begin(spv::OpFunctionParameter).add_seq(data_type, data).commit(&func_);

      auto alloc_var = [&](const SType &type) {
        SType ptr_type = get_pointer_type(type, spv::StorageClassFunction);
        Value ret = new_value(ptr_type, ValueKind::kVariablePtr);
        ib_.begin(spv::OpVariable).add_seq(type, ret, spv::StorageClassFunction).commit(&func_);
      ...
      ib_.begin(spv::OpLoopMerge).add_seq(merge_label, continue_label, spv::LoopControlMaskNone).commit(&func_);
      ...
      ib_.begin(spv::OpIEqual).add_seq(t_bool_, tmp1, tmp0, const_i32_zero_).commit(&func_);
      ib_.begin(spv::OpBranchConditional).add_seq(tmp1, true_label, merge_label).commit(&func_);
      ...
      ib_.begin(spv::OpAtomicCompareExchange).add_seq(t_int32_, tmp9, addr_ptr,
          const_u32_1,
          const_u32_0,
          const_u32_0,
          tmp8, tmp7).commit(&func_);
      ...
      ib_.begin(spv::OpIEqual).add_seq(t_bool_, tmp12, tmp10, tmp11).commit(&func_);
      Value tmp13 = new_value(t_int32_, ValueKind::kNormal);
      ib_.begin(spv::OpSelect).add_seq(t_int32_, tmp13, tmp12, const_i32_one_, const_i32_zero_).commit(&func_);
      store_var(ok, tmp13);
  ...
  void IRBuilder::init_random_function(Value global_tmp_) {
    ...
    ib_.begin(spv::OpVariable).add_seq(t_uint32_, _rand_x_, spv::StorageClassPrivate).commit(&global_);
    ib_.begin(spv::OpVariable).add_seq(t_uint32_, _rand_y_, spv::StorageClassPrivate).commit(&global_);
    ib_.begin(spv::OpVariable).add_seq(t_uint32_, _rand_z_, spv::StorageClassPrivate).commit(&global_);
    ib_.begin(spv::OpVariable).add_seq(t_uint32_, _rand_w_, spv::StorageClassPrivate).commit(&global_);
    ...
    ib_.begin(spv::OpAccessChain).add_seq(gtmp_type, rand_gtmp_, 
        global_tmp_, const_i32_zero_, _1024).commit(&func_header_);
    ...
    ib_.begin(spv::OpAccessChain).add_seq(pint_type, tmp0, gl_global_invocation_id,
                              uint_immediate_number(t_uint32_, 0)).commit(&func_header_);
    ...
    ib_.begin(spv::OpIAdd).add_seq(t_int32_, tmp10, tmp9, _1).commit(&func_header_);
